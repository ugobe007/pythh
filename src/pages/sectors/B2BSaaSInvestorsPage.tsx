...
const B2BSaaSInvestorsPage: React.FC = () => {
  ...
  const loadSectorData = async () => {
    try {
      ...
      setStats({
        ...
        totalRaised: 420 // Placeholder - would calculate from actual data
      });

    } catch (error) {
      console.error('Error loading B2B SaaS sector data:', error);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="container mx-auto px-4 pt-24 pb-16">
        ...
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-500/10 border border-blue-500/20 mb-6">
            ...
            <span className="text-blue-400 font-medium">B2B SaaS Sector</span>
          </div>
          ...
          <h1 className="text-5xl md:text-6xl font-bold mb-6 bg-gradient-to-r from-white via-slate-200 to-slate-400 bg-clip-text text-transparent">
            Top B2B SaaS Investors
          </h1>
          
          <p className="text-xl text-slate-300 max-w-3xl mx-auto mb-12">
            Connect with leading investors powering the future of enterprise software and business solutions
          </p>
        ...
        <div className="mb-16">
          <h2 className="text-3xl font-bold text-white mb-8">Leading B2B SaaS Investors</h2>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {investors.map((investor) => (
                <div key={investor.id} className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl p-6 hover:border-blue-500/50 transition-colors">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <h3 className="text-xl font-bold text-white mb-1">{investor.name}</h3>
                      <span className="text-sm text-blue-400">{investor.type}</span>
                    </div>
                    {investor.total_score && (
                      <div className="px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20">
                        <span className="text-blue-400 font-bold">{investor.total_score}</span>
                      </div>
                    )}
                  </div>
        ...
        <div className="mb-16">
          <h2 className="text-3xl font-bold text-white mb-8">Top B2B SaaS Startups</h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {startups.map((startup) => (
              <div key={startup.id} className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl p-6 hover:border-blue-500/50 transition-colors">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-lg font-bold text-white">{startup.name}</h3>
                  <div className="px-2 py-1 rounded-full bg-blue-500/10 border border-blue-500/20">
                    <span className="text-blue-400 text-sm font-bold">{startup.total_god_score}</span>
                  </div>
                </div>
                <p className="text-sm text-slate-400 line-clamp-3">
                  {startup.solution || startup.problem || 'Innovative B2B SaaS solution'}
                </p>
              </div>
            ))}
          </div>
        ...
        {/* CTA Section */}
        <div className="bg-gradient-to-r from-blue-500/10 via-blue-600/10 to-blue-500/10 border border-blue-500/20 rounded-2xl p-12 text-center">
          <h2 className="text-3xl font-bold text-white mb-4">Find Your Perfect B2B SaaS Investor</h2>
          <p className="text-lg text-slate-300 mb-8 max-w-2xl mx-auto">
            Get AI-powered matches with investors specifically looking for B2B SaaS startups like yours
          </p>
          <a 
            href="/"
            className="inline-flex items-center gap-2 px-8 py-4 rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 text-white font-semibold hover:from-blue-600 hover:to-blue-700 transition-all transform hover:scale-105"
          >
...
export default B2BSaaSInvestorsPage;
