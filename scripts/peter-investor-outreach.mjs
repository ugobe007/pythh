...
function buildEmail(investor, matches) {
  ...
  const opening = `Hi ${firstName(investor)}, my name is Peter with Pythh. We automate your deal flow by matching you with startups that align with your thesis, portfolio, and timing. On that note, I found ${matches.length} startup matches for ${firm}. Check them out. We use math, not magic.`;
  const rows = matches.map((m, i) => `<tr><td style="padding:16px;border-top:1px solid #243047"><strong style="color:#f8fafc">${i + 1}. ${esc(m.name)}</strong><span style="float:right;color:#22c55e;font-family:monospace">${m.match_score} fit</span><div style="color:#94a3b8;font-size:12px;margin-top:5px">${esc([m.stage, ...(Array.isArray(m.sectors) ? m.sectors.slice(0, 2) : [])].filter(Boolean).join(' · '))}</div><div style="color:#64748b;font-size:12px;line-height:1.5;margin-top:7px">${esc(m.match_reason)}</div></td></tr>`).join('');
  const html = `<!doctype html><html><body style="margin:0;background:#080d16;font-family:system-ui,sans-serif"><div style="max-width:620px;margin:auto;padding:40px 20px"><div style="color:#22c55e;font:700 11px monospace;letter-spacing:.12em">PYTHH · INVESTOR MATCHES</div><h1 style="color:#f8fafc;font-size:24px">${matches.length} startups aligned with ${esc(firm)}</h1><p style="color:#94a3b8;line-height:1.65">${esc(opening)}</p><table width="100%" cellspacing="0" style="background:#0f172a;border:1px solid #243047;border-radius:10px">${rows}</table><div style="text-align:center;margin-top:26px"><a href="${url}" style="display:inline-block;background:#22c55e;color:#03130b;padding:13px 24px;border-radius:8px;text-decoration:none;font-weight:700">See my startup matches</a></div><p style="color:#475569;font-size:11px;text-align:center;margin-top:24px">Peter at Pythh · We use math, not magic.</p>${unsubscribe ? `<p style="text-align:center;font-size:10px"><a style="color:#64748b" href="${unsubscribe}">Unsubscribe from investor match emails</a></p>` : ''}</div></body></html>`;
  const text = `${opening}\n\n${matches.map((m, i) => `${i + 1}. ${m.name} — ${m.match_score} fit\n${m.match_reason}`).join('\n\n')}\n\nSee your matches: ${url}\n\nPeter at Pythh\nWe use math, not magic.${unsubscribe ? `\n\nUnsubscribe: ${unsubscribe}` : ''}`;
