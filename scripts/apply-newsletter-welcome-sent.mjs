#!/usr/bin/env node
/**
 * Add welcome_sent_at on newsletter_subscribers.
 *   node scripts/apply-newsletter-welcome-sent.mjs --apply
 */
...
const sql = readFileSync(new URL('../supabase/migrations/20260909010000_newsletter_subscriber_welcome_sent.sql', import.meta.url), 'utf8');
...
for (const [fn, args] of attempts) {
  ...
  if (!error && data?.success !== false) {
    const verify = await db.from('newsletter_subscribers').select('email, welcome_sent_at').limit(1);
