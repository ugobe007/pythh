/**
 * PYTHIA Email Inference Engine — Reusable Template
 *
 * Priority order (most → least common at VC/B2B firms):
 *   1. firstname@domain
...
// ── Extend this map with domain-specific firms ──────────────────────────────
...
export function generateEmailVariants(
  ...
  const personalVariants: EmailVariant[] = [
    { address: `${fn}@${domain}`,       pattern: "firstname@",             confidence: "high",   type: "personal" },
    { address: `${fn}.${ln}@${domain}`, pattern: "firstname.lastname@",    confidence: "high",   type: "personal" },
    { address: `${fi}.${ln}@${domain}`, pattern: "firstinitial.lastname@", confidence: "medium", type: "personal" },
    { address: `${ln}@${domain}`,       pattern: "lastname@",              confidence: "medium", type: "personal" },
    { address: `${fi}${ln}@${domain}`,  pattern: "firstinitiallastname@",  confidence: "low",    type: "personal" },
  ];
...
export function inferInvestorEmails(fullName: string, firmName: string, customDomain?: string): InvestorEmailProfile {
