...
test('save signup lands on the account profile, not the newsletter', () => {
  ...
  assert.match(hub, /We email the five names from the same Daily Brief sender \(hello@orbital-ai\.io\)/);
  ...
  assert.match(hub, /hello@orbital-ai\.io/);
  ...
  assert.match(account, /Your startup/);
