...
test('rejects unsafe or non-financing scraper classifications and separates debt', () => {
  ...
  assert.equal(classifyFundingEvidence({ ...base, source_title: 'OpenPayd Secures 43 US Licenses Ahead of Nasdaq Listing' }).reason, 'non_financing_headline');
...
test('historical funding search defaults to inference engine, not Gemini', () => {
  ...
  assert.match(script, /providerArg === 'anthropic'/);
  assert.match(script, /providerArg === 'cascade'/);
  assert.match(script, /providerArg === 'ontology'/);
  assert.match(script, /openai_web_search/);
  assert.match(script, /anthropic_web_search/);
  ...
  assert.match(script, /RETIRED_ANTHROPIC_SEARCH_MODELS/);
  assert.match(script, /claude-sonnet-4-20250514/);
  assert.match(script, /liveAnthropicSearchModels/);
  assert.match(script, /deadAnthropicModels/);
