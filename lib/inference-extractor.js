 ...
 * Uses verb-centric extraction (verb anchors the sentence; extract subject/object).
 */
function extractCompanyNameFromHeadline(text) {
  const { extractCompanyNameVerbCentric } = require('./verbCentricExtractor');
  const verbCentric = extractCompanyNameVerbCentric(text);
  if (verbCentric) return verbCentric;
  // Fallback: simple patterns
  if (!text || typeof text !== 'string' || text.length < 8) return null;
  const m = text.match(/^([A-Z][A-Za-z0-9\s&.-]{2,50}?)\s+(?:raises?|secures?|closes?|bags?|lands?|announces?|gets?|receives?)\s+/i);
  if (m) return m[1].trim();
  const m2 = text.match(/(?:startup|company)\s+([A-Z][A-Za-z0-9\s&.-]{2,40}?)\s+(?:raises?|secures?)/i);
  if (m2) return m2[1].trim();
  const m3 = text.match(/^([A-Z][A-Za-z0-9\s&.-]{2,50}?)\s+(?:launches?|partners?|acquires?|unveils?|releases?)\s+/i);
  if (m3) return m3[1].trim();
  const m4 = text.match(/([A-Z][A-Za-z0-9\s&.-]{2,40}?)\s*[,(\s]\s*(?:a\s+)?(?:fintech|healthtech|ai|saas|startup)/i);
