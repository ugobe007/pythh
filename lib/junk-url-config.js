...
const JUNK_DOMAINS = new Set([
  ...
  // ── Investor / portfolio sites (not startup canonical domains) ───────────
  'initialized.com',
  'shastaventures.com',
  'villageglobal.vc',
...
function isJunkUrl(url) {
  ...
  try {
    ...
    // 3. Obvious article paths (2 segments): /article/..., /story/..., /news/...
    const p = u.pathname;
    if (/\/(article|articles|story|stories|posts?|news)\b/i.test(p)) return true;

    // 4. Deep article-style path: 3+ non-empty segments
