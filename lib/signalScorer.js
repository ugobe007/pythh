...
const { getActiveSignalWeightConfigSync } = require('./signalWeightConfig');

const SIGNAL_CLASS_PRIORITY = require('./signalOntology').SIGNAL_CLASS_PRIORITY;

function getPriorityWeights() {
  return getActiveSignalWeightConfigSync().classPriorityWeights;
}
...
function scoreSignal(signal, perspective = 'general') {
  ...
  const priorityWeight  = getPriorityWeights()[signal.primary_signal] ?? 0.5;
