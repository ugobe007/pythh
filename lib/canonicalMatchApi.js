 ...
 *   - never email / email_best_guess / email_candidates (contactable flag only)
 */
...
const { filterAndRankByFundingLifecycle, getStartupFundingStage } = require('./fundingLifecycleFit');
const { calculateInvestorFitness } = require('./investorFitness');
const { shapeRecentDeals, investorHasContact } = require('./recentInvestorDeals');
...
function shapeInvestorForApi(investor) {
  ...
  return {
    ...
    total_investments: Number.isFinite(Number(investor.total_investments))
      ? Number(investor.total_investments)
      : null,
    last_investment_date: investor.last_investment_date
      ? String(investor.last_investment_date)
      : null,
    recent_deals: shapeRecentDeals(investor, 5),
  };
}
...
function shapeMatchForApi(row) {
  ...
  const fitness = calculateInvestorFitness({
    ...row,
    investor: investorJoined,
  });
  ...
  return {
    ...
    funding_lifecycle_fit: row.funding_lifecycle_fit ?? null,
    fitness_score: fitness.score,
    fitness_confidence: fitness.confidence,
    fitness_factors: fitness.factors,
    fitness_components: fitness.components,
    fitness_behavior_sample_size: fitness.behavior_sample_size,
    fitness_methodology_version: fitness.methodology_version,
    investor,
...
function buildPreviewMatchList(rawMatches, mixOptions = {}) {
  const lifecycleMatches = mixOptions.startup
    ? filterAndRankByFundingLifecycle(rawMatches, mixOptions.startup)
    : rawMatches;
  const mixed = buildMixedInvestorShortlist(lifecycleMatches, mixOptions);
...
function resolvePreviewMixOptions(startup, mixParamRaw, total = 10) {
  ...
  return {
    ...
    startup,
    fundingStage: getStartupFundingStage(startup),
  };
}
...
function summarizeShortlistMix(matches, mixOptions) {
  ...
  return {
    ...
    funding_stage: mixOptions.fundingStage || null,
    lifecycle_filtered: Boolean(mixOptions.fundingStage),
  };
}
