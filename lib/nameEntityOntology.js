...
const ESTABLISHED_BRANDS = new Set([
  ...
  // Alphabet = Google's public parent (abc.xyz); Alphabet Ventures → GV (investor track)
  'microsoft', 'apple', 'google', 'alphabet', 'amazon', 'meta', 'netflix',
...
/**
 * Public holding-company / parent tokens that dominate RSS as subjects.
 * Used for trailing-token bleed ("Dollar Debt Alphabet") — narrower than
 * ESTABLISHED_BRANDS so we don't reject "... Amazon" product phrases lightly.
 * Alphabet Ventures / Alphabet VC are handled as investor by the logic engine.
 */
const MEGACORP_PARENT_TOKENS = new Set([
  'alphabet', // Google parent (abc.xyz); not a fundable startup
  'google',
  'microsoft',
  'meta',
  'amazon',
...
function classifyEntityType(name) {
  ...
  // Trailing megacorp-parent bleed: "Dollar Debt Alphabet", "Sources Alphabet"
  // Only for holding-company tokens that dominate RSS as subjects (not product brands).
  if (words.length >= 2 && MEGACORP_PARENT_TOKENS.has(words[words.length - 1])) return 'brand';
