...
function scoreVideoCandidate({ entityName, entityDomain, title, description, channelTitle, kind }) {
  ...
  if (intent.test(haystack)) { score += 0.18; reasons.push('content_intent'); }
  if (channel && (channel.includes(name) || name.includes(channel))) { score += 0.17; reasons.push('entity_channel'); }
