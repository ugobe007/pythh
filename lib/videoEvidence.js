...
function scoreVideoCandidate({ entityName, entityDomain, title, description, channelTitle, channelDescription, channelCustomUrl, kind }) {
  const name = normalizeName(entityName);
  const rawMetadata = `${title || ''} ${description || ''} ${channelTitle || ''} ${channelDescription || ''} ${channelCustomUrl || ''}`.toLowerCase();
  const haystack = normalizeName(`${title || ''} ${description || ''}`);
  const channel = normalizeName(channelTitle);
  const phrase = (value) => value && new RegExp(`(?:^|\\s)${value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(?:\\s|$)`).test(haystack);
  const hostname = String(entityDomain || '').toLowerCase().replace(/^www\./, '').replace(/\/$/, '');
  const channelMetadata = `${channelDescription || ''} ${channelCustomUrl || ''}`.toLowerCase();
  const channelDomainProof = Boolean(hostname && channelMetadata.includes(hostname));
  const alias = normalizeName(name.replace(/\b(partners?|ventures?|venture capital|capital|fund)\b/gi, ' '));
  const exactPhrase = phrase(name) || (channelDomainProof && alias.length >= 4 && phrase(alias));
  if (!exactPhrase) return { score: 0, reasons: ['entity_name_missing'] };
  const nameTokens = name.split(/\s+/).filter(Boolean);
  const channelIdentity = Boolean(channelDomainProof || (channel && (channel === name || channel.startsWith(`${name} `) || name.startsWith(`${channel} `))));
  ...
  const fullDomainMention = Boolean(hostname && rawMetadata.includes(hostname));
  if (nameTokens.length === 1 && !fullDomainMention) {
    return { score: 0, reasons: ['ambiguous_single_token_without_domain_evidence'] };
  }
  if (hasIntent) { score += 0.18; reasons.push('content_intent'); }
  if (channelIdentity) { score += 0.17; reasons.push('entity_channel'); }
  if (channelDomainProof) reasons.push('official_channel_domain');
  if (fullDomainMention) { score += 0.1; reasons.push('domain_identity'); }
...
module.exports = { youtubeEmbedUrl, normalizeConfidence, normalizeBoolean, evidenceHash, validateSnippet, graphPredicates, scoreVideoCandidate, discoveryQueries };
