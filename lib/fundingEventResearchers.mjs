...
const RAISED_AT_MONEY_RE =
  /(?:raises?|raised|more money)\s+(?:[^.!?]{0,48}?\s)?at\s+\$?(\d+(?:\.\d+)?)\s*(million|m|billion|b)\b/i;
const EXPLICIT_RAISE_THEN_VAL_RE =
  /(?:raises?|raised|secures?|secured|closes?|closed)\s+\$?(\d+(?:\.\d+)?)\s*(million|m|billion|b)\s+at\s+/i;

export const FUNDING_INTEL_VERSION = 'funding-intel-v4';
export const RAISE_CLUSTER_WINDOW_DAYS = 5;
...
export function researchRoundEconomics({ event = {}, text = '' } = {}) {
  ...
  const atMoney = text.match(RAISED_AT_MONEY_RE);
  if (atMoney && !EXPLICIT_RAISE_THEN_VAL_RE.test(text)) {
    const atUsd = parseMoney(atMoney[1], atMoney[2]);
    if (atUsd) {
      if (!valuationUsd) {
        valuationUsd = atUsd;
        valuationKind = valuationKind || 'unknown';
      }
      if (amountUsd === atUsd) {
        amountUsd = null;
        amountRaw = null;
  ...
  if (amountUsd && valuationUsd && amountUsd >= valuationUsd) {
    amountUsd = null;
    amountRaw = null;
    currency = null;
  }
...
export function eventResearchable(event = {}) {
  const name = String(event.startup_name_raw || '').trim();
  const title = String(event.source_title || '');
  if (!name) return { ok: false, reason: 'empty_name' };
  if (HEADLINE_VERB_NAME_RE.test(name)) return { ok: false, reason: 'headline_verb_name' };
  if (title && !FUNDING_HEADLINE_RE.test(title)) return { ok: false, reason: 'not_a_raise' };
  const former = title.match(FORMER_ORG_STAFFERS_RE);
  if (former && name.toLowerCase() === former[1].toLowerCase()) {
    return { ok: false, reason: 'former_org_not_startup' };
  }
  const valid = isValidStartupName(name);
  if (!valid.isValid) return { ok: false, reason: valid.reason || 'invalid_name' };
...
function startupName(event = {}) {
  return String(event.startup_name_raw || '').toLowerCase().replace(/\s+/g, ' ').trim();
}

function raiseAmount(event = {}) {
  const n = Number(event.amount_usd);
  return Number.isFinite(n) && n > 0 ? n : 0;
}

function raiseTime(event = {}) {
  return Date.parse(event.announced_at || event.occurred_at || 0) || 0;
}

export function eventsAreSameRaise(a = {}, b = {}, windowDays = RAISE_CLUSTER_WINDOW_DAYS) {
  const nameA = startupName(a);
  if (!nameA || nameA !== startupName(b)) return false;
  const amountA = raiseAmount(a);
  const amountB = raiseAmount(b);
  if (!amountA || amountA !== amountB) return false;
  const tA = raiseTime(a);
  const tB = raiseTime(b);
  if (!tA || !tB) return true;
  return Math.abs(tA - tB) <= windowDays * 24 * 60 * 60 * 1000;
}

export function groupEventsByRaise(events = [], windowDays = RAISE_CLUSTER_WINDOW_DAYS) {
  const keyed = new Map();
  events.forEach((event, index) => {
    const name = startupName(event);
    const amount = raiseAmount(event);
    const key = amount ? `${name}|${amount}` : `${eventDedupeKey(event)}#${event.id || index}`;
  ...
  for (const list of keyed.values()) {
    if (!raiseAmount(list[0])) {
      clusters.push(list);
      continue;
    }
    const sorted = list.slice().sort((a, b) => raiseTime(a) - raiseTime(b) || String(a.id || '').localeCompare(String(b.id || '')));
    let current = [];
    for (const event of sorted) {
      if (current.length && !eventsAreSameRaise(event, current[current.length - 1], windowDays)) {
...
function clusterRecency(cluster = []) {
  return Math.max(0, ...cluster.map((event) => raiseTime(event)));
}
...
export function pickRichestPerRaise(events = []) {
  return groupEventsByRaise(events)
    .sort((a, b) => clusterRecency(b) - clusterRecency(a))
    .map((cluster) => pickRichestEvent(cluster));
}
...
export function selectResearchEvents(candidates = [], { limit = 80, isDone = () => false } = {}) {
  const clusters = groupEventsByRaise(candidates)
    .sort((a, b) => clusterRecency(b) - clusterRecency(a));
  ...
  return {
    ...
    clusters: clusters.map((members) => ({
      key: raiseClusterKeyFromGroup(members),
      canonical: pickRichestEvent(members),
      members,
    })),
  };
}

export function raiseClusterKeyFromGroup(events = []) {
  const members = (events || []).filter(Boolean);
  if (!members.length) return '';
  const earliest = members.slice().sort((a, b) => (
    raiseTime(a) - raiseTime(b) || String(a.id || '').localeCompare(String(b.id || ''))
  ))[0];
  return `${startupName(earliest)}|${raiseAmount(earliest) || ''}|${String(earliest.announced_at || earliest.occurred_at || '').slice(0, 10)}`;
}

export function raiseClusterPatches(cluster = {}, briefing = null) {
  const members = cluster.members || [];
  const canonical = cluster.canonical || pickRichestEvent(members);
  const key = cluster.key || raiseClusterKeyFromGroup(members);
  if (!canonical?.id || !key) return [];
  const siblingIds = members.filter((row) => row.id && row.id !== canonical.id).map((row) => row.id);
  const stampedAt = new Date().toISOString();
  return members.map((event) => {
    const isCanonical = event.id === canonical.id;
    const raiseCluster = {
      key,
      role: isCanonical ? 'canonical' : 'sibling',
      canonical_event_id: canonical.id,
      stamped_at: stampedAt,
      ...(isCanonical ? { sibling_ids: siblingIds } : {}),
    };
    if (isCanonical && briefing) {
      const intel = eventPatchFromBriefing(event, briefing);
      return {
        id: event.id,
        ...intel,
        metadata: {
          ...intel.metadata,
          raise_cluster: raiseCluster,
        },
      };
    }
    const meta = event.metadata && typeof event.metadata === 'object' ? { ...event.metadata } : {};
    return { id: event.id, metadata: { ...meta, raise_cluster: raiseCluster } };
  });
}
...
function previewWhyRank(row = {}) {
  if (row.why && row.why !== 'unspecified') return 2;
  if (row.valuation_usd) return 1;
  return 0;
}
...
export function uniquePreview(rows = [], limit = 8) {
  ...
  for (const row of rows) {
    ...
    if (previewWhyRank(row) > previewWhyRank(current)) best.set(key, row);
  }
  return order.map((key) => best.get(key)).slice(0, limit);
}
