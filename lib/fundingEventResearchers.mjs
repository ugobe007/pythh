...
export const FUNDING_INTEL_VERSION = 'funding-intel-v1';
...
const USE_OF_PROCEEDS_RE =
  /\bto\s+(?:scale|expand|grow|build|hire|launch|accelerate|fuel|fund|commercialize|enter)\s+[^.!?]{6,140}/i;
...
export function researchRoundEconomics({ event = {}, text = '' } = {}) {
  ...
  let amountUsd = Number(funding.funding_amount) || Number(event.amount_usd) || null;
  let amountRaw = amountUsd;
  let currency = amountUsd ? 'USD' : null;
...
export function researchWhyFunded({ text = '' } = {}) {
  ...
  const proceedsMatch = text.match(USE_OF_PROCEEDS_RE);
  ...
  const gaps = [];
  if (triggers.primary === 'unspecified' && !useOfProceeds) gaps.push('why_funded');
  return {
    researcher: 'why_funded',
    confidence: triggers.primary === 'unspecified' ? (useOfProceeds ? 0.45 : 0.12) : 0.72,
    findings: [
      triggers.primary !== 'unspecified' ? `why ${triggers.primary}` : null,
...
export function composeBriefing({ event = {}, reports = {} } = {}) {
  ...
  const filled = RESEARCHER_IDS.length - researcherReports.filter((row) => (row.gaps || []).length === (row.findings || []).length && !(row.findings || []).length).length;
