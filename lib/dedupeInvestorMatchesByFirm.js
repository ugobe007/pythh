...
const { isNonInvestorAggregator } = require('./investorAggregatorBlocklist');
...
function dedupeInvestorMatchesByFirm(matches, limit = 5) {
  ...
  const lifecycleRank = { exact: 3, compatible: 2, inferred: 1, unknown: 0 };
  const sorted = [...matches].sort((a, b) => {
    const lifecycleDelta =
      (lifecycleRank[b.funding_lifecycle_fit?.level] || 0) -
      (lifecycleRank[a.funding_lifecycle_fit?.level] || 0);
    return lifecycleDelta || (Number(b.match_score) || 0) - (Number(a.match_score) || 0);
  });
  const seen = new Set();
  const out = [];
