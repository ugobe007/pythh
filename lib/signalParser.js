...
const _VERB_TENSE_RE = VERB_TENSE_MAP; // [RegExp, meta][]
...
// ─── VERB TENSE / ASPECT (completed past vs present stance vs future) ─────────
/** @returns {{ aspect: string, weight: number, meaning: string }[]} */
function detectVerbTense(text) {
  const byAspect = new Map();
  for (const [pattern, meta] of _VERB_TENSE_RE) {
    if (!pattern.test(text)) continue;
    const prev = byAspect.get(meta.aspect);
    if (!prev || (meta.weight > prev.weight)) byAspect.set(meta.aspect, { ...meta });
  }
...
function inferMeanings({ actor, actions, modality, intensity, posture, context, intent, verbTense, text }) {
  const meanings = new Set();

  // ── Hiring layer ────────────────────────────────────────────────────────────
  const isHiring = actions.some(
    a => a.action_tag?.includes('hiring') || a.action_tag?.includes('recruit')
  );
  if (isHiring) {
    ...
    if (actions.some(a => a.signal_class === 'gtm_hiring_signal')) meanings.add('gtm_headcount_build');
    if (actions.some(a => a.signal_class === 'engineering_hiring_signal')) meanings.add('engineering_headcount_build');
  }

  if (actions.some(a => a.signal_class === 'diligence_signal')) {
    meanings.add('formal_investor_diligence');
    if (actions.some(a => a.action_tag === 'action_investor_diligence' && a.base_certainty >= 0.92)) {
      meanings.add('late_stage_financing_process');
    }
  }
  ...
  // ── Verb-tense narrative (completed past + present stance ± future projection) ─
  const tenseAspects = new Set((verbTense || []).map((v) => v.aspect));
  const tx = text || '';
  const hasInvestorOrRoundLanguage = actions.some((a) =>
    ['investor_interest_signal', 'fundraising_signal', 'acquisition_signal'].includes(a.signal_class)
  );
  const hasDealVerbInText = /\b(funded|co-?funded|backed|invested in|led the round|co-?led|raised|closed|secured)\b/i.test(tx);
  const narrativeFinanceContext = hasInvestorOrRoundLanguage || hasDealVerbInText;
  if (
    narrativeFinanceContext
    && tenseAspects.has(VERB_TENSE_ASPECT.COMPLETED_PAST)
    && tenseAspects.has(VERB_TENSE_ASPECT.PRESENT_ANALYTIC)
  ) {
    meanings.add('investor_narrative_past_deal_and_present_thesis');
  }
  if (
    narrativeFinanceContext
    && tenseAspects.has(VERB_TENSE_ASPECT.COMPLETED_PAST)
    && tenseAspects.has(VERB_TENSE_ASPECT.PRESENT_ANALYTIC)
    && tenseAspects.has(VERB_TENSE_ASPECT.FUTURE_PROJECTION)
  ) {
    meanings.add('investor_narrative_past_present_future_arc');
  }

  // ── Multi-actor competitive revelation (e.g. OEM + supplier + parallel competitor) ─
  // "had no idea that X was doing the same thing, so they dropped… and decided to build…"
  if (
    tenseAspects.has(VERB_TENSE_ASPECT.EPISTEMIC_PAST)
    && tenseAspects.has(VERB_TENSE_ASPECT.PAST_CONTINUOUS)
    && /\bso they\b/i.test(tx)
    && (
      actions.some((a) => ['action_exiting', 'action_building'].includes(a.action_tag))
      || /\b(dropped|decided to build)\b/i.test(tx)
    )
  ) {
    meanings.add('strategic_pivot_after_competitive_blind_spot');
  }
...
function parseSignal(text, options = {}) {
  ...
  const verb_tense  = detectVerbTense(cleaned);
  ...
  const inferred_meanings = isNegated ? [] : inferMeanings({
    actor, actions, modality, intensity, posture, context, intent, verbTense: verb_tense, text: cleaned,
  });
