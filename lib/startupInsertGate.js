...
const { isJunkUrl } = require('./junk-url-config');
...
async function insertDiscovered(data, opts = {}) {
  ...
  if (data.website && isJunkUrl(data.website)) {
    onReject(name, 'non_canonical_website', 'discovered');
    return { ok: false, error: 'invalid_website: publisher, directory, investor, or article URL' };
  }
...
async function insertStartupUpload(record, opts = {}) {
  ...
  if (record.website && isJunkUrl(record.website)) {
    onReject(name, 'non_canonical_website', 'startup_uploads');
    return { ok: false, error: 'invalid_website: publisher, directory, investor, or article URL' };
  }
