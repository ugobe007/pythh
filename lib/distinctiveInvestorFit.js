...
const THESIS_STOPWORDS = new Set([
  'about', 'across', 'after', 'before', 'being', 'build', 'building', 'capital',
  'companies', 'company', 'early', 'every', 'first', 'founded', 'founder', 'fund',
  'funds', 'global', 'growth', 'invest', 'investing', 'investment', 'investments',
  'investor', 'investors', 'leading', 'market', 'other', 'platform', 'portfolio',
  'product', 'series', 'software', 'stage', 'startup', 'startups', 'technology',
  'their', 'these', 'those', 'venture', 'ventures', 'which', 'while', 'world',
]);
...
function thesisOverlap(startup, investor) {
  ...
  for (const word of words) {
    if (seen.has(word) || THESIS_STOPWORDS.has(word)) continue;
    seen.add(word);
...
function distinctiveFitScore(startup, investor) {
  ...
  const startupSecs = [...new Set(sectorList(startup?.sectors))];
  const invSecs = [...new Set(sectorList(investor?.sectors))];
  ...
  if (specific.length) {
    // Count only specific sectors in breadth so generic tags don't dilute specialist scores
    const specificBreadth = Math.max(invSecs.filter((s) => !GENERIC_SECTORS.has(s)).length, 1);
    score += specific.length * 32 + (specific.length / specificBreadth) * 40;
  } else if (direct.length) {
    // A fund that only lists this generic sector still outranks a 10-sector generalist.
    score += 22 + (direct.length / breadth) * 50;
    ...
    try {
      ...
      if (related?.isRelated) score += Math.min(10, (related.score || 0) / 4);
    } catch {
      /* taxonomy optional */
    }
  }
  ...
  try {
    ...
    const stageDelta = Number(calculateStageInvestorFitAdjustment(startup, investor).delta) || 0;
    // Stage is a tie-break. A large seed bonus was putting the same angels on every early startup.
    score += Math.max(-6, Math.min(6, stageDelta));
  } catch {
    /* stage optional */
  }
