...
function investorHasContact(investor) {
  ...
  const looksEmail = (value) => typeof value === 'string' && value.includes('@') && value.length > 5;
  if (looksEmail(investor.email) || looksEmail(investor.email_best_guess)) return true;
  const candidates = Array.isArray(investor.email_candidates) ? investor.email_candidates : [];
  return candidates.some((entry) => {
    if (typeof entry === 'string') return looksEmail(entry);
    return looksEmail(entry?.address || entry?.email);
  });
}
...
function resolveInvestorEmail(investor) {
  ...
  const looksEmail = (value) => typeof value === 'string' && value.includes('@') && value.length > 5;
  if (looksEmail(investor.email)) return { address: investor.email.trim(), type: 'verified' };
  if (looksEmail(investor.email_best_guess)) {
