...
const { isOperatorFounderInvestor } = require('./operatorFounderInvestors');
const { evaluateFundingLifecycleFit, getStartupFundingStage } = require('./fundingLifecycleFit');
...
function getStartupStageBand(stage) {
  if (stage && typeof stage === 'object') {
    const lifecycleStage = getStartupFundingStage(stage);
    if (lifecycleStage === 'pre-seed' || lifecycleStage === 'seed' || lifecycleStage === 'early') return 'early';
    if (lifecycleStage === 'series-a') return 'mid';
    if (lifecycleStage === 'series-b' || lifecycleStage === 'series-c' || lifecycleStage === 'growth') return 'late';
    stage = stage.stage;
  }
...
function calculateStageInvestorFitAdjustment(startup, investor) {
  const startupBand = getStartupStageBand(startup);
  const profile = getInvestorStageProfile(investor);
  const lifecycle = evaluateFundingLifecycleFit(startup, investor);
  ...
  // Successful founders who invest + write publicly (Altman/Chesky/… pattern)
  if (startupBand === 'early' && isOperatorFounderInvestor(investor)) delta += 6;
  if ((startupBand === 'mid' || startupBand === 'early') && isOperatorFounderInvestor(investor)
      && investor.signals?.top_themes?.length) {
    delta += 2;
  }
  ...
  // Funding lifecycle is the gate within the broad "early" band:
  // pre-seed startups rank pre-seed investors; seed startups rank seed investors.
  if (lifecycle.level === 'exact') delta = Math.max(delta, lifecycle.delta);
  if (!lifecycle.eligible) delta = Math.min(delta, lifecycle.delta);

  const partnerAngel = startupBand === 'early' && isPartnerAngelInvestor(investor);
  const operatorFounder = isOperatorFounderInvestor(investor);
  let note;
  if (operatorFounder && delta >= 8) {
    note = `Stage fit: operator / successful-founder investor for ${startupBand}-stage startup`;
  } else if (partnerAngel && delta >= 8) {
