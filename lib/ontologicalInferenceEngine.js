...
function extractSlotEntities(text, frameType) {
  const objects = [];
  const seen = new Set();
  ...
  // Fallback: capitalized spans not already captured
  let cap;
  while ((cap = CAPITALIZED_NAME_RE.exec(text)) !== null) {
    ...
    seen.add(name.toLowerCase());
    const role = inferEntityType(name) === 'INVESTOR' ? 'investor' : 'entity';
    objects.push({
      name,
      entity_type: inferEntityType(name, role),
      role,
      span: cap[1],
      confidence: 0.55,
