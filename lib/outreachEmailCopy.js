...
function humanizeWhyYouMatchForOutreach(raw, ctx = {}) {
  ...
  return `${firm} backs ${matchSector} companies at ${matchStage}; that overlaps directly with ${startupName}'s market and round.`;
}
...
function cleanPitchLine(startup, sector) {
  ...
  const raw = startup?.pitch || startup?.description || startup?.tagline || `We're building in ${sector}.`;
  const withoutDuplicateName = String(raw)
    .replace(new RegExp(`^${name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*[—–:-]\\s*`, 'i'), '')
    .trim();
  return truncateAtSentence(withoutDuplicateName, 300);
}
...
function buildColdEmail(startup, investor, doc, match, options = {}) {
  ...
  const materials = startup?.deck_filename
    ? 'I can send the deck and a concise rollout plan in advance.'
  ...
  const lines = [
    ...
    '',
    `Would a 20-minute conversation be worthwhile? ${materials}`,
    '',
    'Best,',
    founderName,
    website,
