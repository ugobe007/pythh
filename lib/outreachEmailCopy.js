...
function buildInvestorFitLine(startup, investor, match, ctx = {}) {
  const name = startup?.name || 'The company';
  ...
  const thesis = String(investor?.investment_thesis || investor?.bio || '').toLowerCase();
  ...
  const overlaps = [];
  const themeRules = [
    [/\b(robot|physical ai|automation|autonomous)\b/, 'physical AI'],
    [/\b(infrastructure|platform|developer tool|operating system)\b/, 'infrastructure'],
    [/\b(data|memory|intelligence|analytics)\b/, 'data and intelligence'],
    [/\b(enterprise|b2b|commercial)\b/, 'enterprise deployment'],
    [/\b(frontier|deep tech|hard tech|technical breakthrough)\b/, 'frontier technology'],
  ];
  for (const [pattern, label] of themeRules) {
    if (pattern.test(startupText) && pattern.test(thesis)) overlaps.push(label);
  }
