...
const GARBAGE_NAME_PATTERNS = [
  ...
  /^Bio\s+/i,
  /\bPersonal\s*$/i,
  /\b(?:launches?|launched|closes?|closed|backs?|backed|invests|invested|investing|leads|led|participates?|participated)\b/i,
...
function looksLikeCleanFirmName(trimmed) {
  ...
  // Allow an interior "&" and simple hyphenated brands ("Invest-NL").
  if (/^[A-Z][A-Za-z]+-[A-Z][A-Za-z0-9]{0,12}$/.test(core)) {
    return true;
  }
  if (
    /^[A-Z][A-Za-z]{0,20}(?:\s+(?:&|[A-Z][A-Za-z]{1,20})){0,5}$/.test(core) &&
...
function isGarbageInvestorName(name) {
  ...
  // Sovereign wealth funds (Temasek, GIC, …) and "Country's SWF" forms are never junk.
  // Geo fencing still rejects bare country labels elsewhere; do not delete the SWF brand.
  try {
    const {
      stripInvestorHeadlineNoise,
      isSovereignWealthFundBrand,
    } = require('../server/lib/fundingEvidenceLedger.js');
    const cleaned = stripInvestorHeadlineNoise(trimmed);
    if (isSovereignWealthFundBrand(trimmed) || isSovereignWealthFundBrand(cleaned)) {
      return false;
    }
  } catch {
    // Keep heuristics usable if ledger import fails in odd runners.
  }
  ...
  if (/^(?:Data|Life)\s+Scientist\b/i.test(trimmed)) return true;
  if (/^Bio\s+/i.test(trimmed)) return true;
  if (/\bPersonal\s*$/i.test(trimmed)) return true;
  // Verb forms only — do not flag firm tokens like "Invest-NL" / "Nextech Invest".
  if (/\b(?:launches?|launched|closes?|closed|backs?|backed|invests|invested|investing|leads|led|participates?|participated)\b/i.test(trimmed)) return true;
  ...
  // Digit-leading brands without a VC suffix ("360 ONE", "8VC")
  if (/^[0-9]+(?:\s+[A-Za-z][A-Za-z0-9&]*)+$/.test(core) && !hasGluedRoleSuffix(core)) {
    return false;
  }
 ...
 *
 * Digit-leading brands (468 Capital, 8VC, 360 ONE) are NOT hard junk — same
 * fast-pass as isGarbageInvestorName.
 */
function isHardJunkInvestorName(name) {
  ...
  const core = trimmed.replace(/\s*\([^)]{1,40}\)\s*$/, '').trim();
  // Digit / alphanumeric VC brands — do not treat /^\d/ as hard junk for these.
  if (VC_FIRM_SUFFIXES.test(core) || LEGAL_ENTITY_SUFFIX.test(core)) {
    return hasGluedRoleSuffix(core);
  }
  if (/\b(Startups?|Global|Combinator)\s*$/i.test(core) && !hasGluedRoleSuffix(core)) {
