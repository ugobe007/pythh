...
const GARBAGE_NAME_PATTERNS = [
  ...
  /^Bio\s+/i,
  /\bPersonal\s*$/i,
  /\b(?:launches?|launched|closes?|closed|backs?|backed|invests?|invested|leads?|led|participates?|participated)\b/i,
...
function isGarbageInvestorName(name) {
  ...
  if (/\b(?:launches?|launched|closes?|closed|backs?|backed|invests?|invested|leads?|led|participates?|participated)\b/i.test(trimmed)) return true;
