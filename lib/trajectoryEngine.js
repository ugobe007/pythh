...
const VELOCITY_WEIGHTS = {
  ...
  hiring_signal:             0.65,
  gtm_hiring_signal:         0.68,
  engineering_hiring_signal: 0.66,
  gtm_signal:                0.65,
  product_signal:            0.60,
...
const SIGNAL_DOMAIN = {
  ...
  gtm_hiring_signal:         'growth',
  engineering_hiring_signal: 'growth',
...
const TRAJECTORY_PATTERNS = [
  ...
  // ── Narrative trajectories (multi-step chains → prediction) ─────────────────
  // Human storylines use coarse signal ontology today; finer roles (e.g. “VP Sales”,
  // “data room”) require richer extraction — see docs/PYTHH_TRAJECTORY_ENGINE.md.

  // Story: Hiring → sales/GTM build → enterprise motion → diligence → financing
  {
    id:          'hiring_ladder_to_finance',
    label:       'Hiring ladder → diligence → fundraise',
    description:
      'General hiring, then GTM/sales motion, enterprise go-to-market, investor interest, active financing — mirrors hiring → sales hiring → enterprise → data room → raise when signals are sparse',
    type:        'fundraising',
    sequence: [
      { classes: ['hiring_signal', 'engineering_hiring_signal'],                  max_gap_days: 90 },
      { classes: ['gtm_hiring_signal', 'gtm_signal', 'hiring_signal'],          max_gap_days: 90 },
      { classes: ['enterprise_signal', 'gtm_signal', 'gtm_hiring_signal'],      max_gap_days: 90 },
      { classes: ['diligence_signal', 'investor_interest_signal'],              max_gap_days: 75 },
      { classes: ['fundraising_signal'],                                        max_gap_days: 60 },
    ],
    predicted_next_moves: [
      'complete_diligence',
      'close_funding_round',
      'hire_vp_sales',
      'expand_enterprise_pipeline',
    ],
    stage_exit: 'fundraising_active',
    who_cares: { investors: true, vendors: true, recruiters: true },
    strength: 0.82,
  },

  // Story: Pilot / demand → RFP / pain → vendor eval → deployment
  {
    id:          'pilot_rfp_deploy',
    label:       'Pilot → RFP → vendor selection → deployment',
    description:
      'Validation or pilot demand, structured buying signals, budget/committee, then rollout-scale signals',
    type:        'buying',
    sequence: [
      { classes: ['demand_signal', 'product_signal', 'buyer_pain_signal'],        max_gap_days: 90 },
      { classes: ['buyer_pain_signal', 'buyer_signal'],                         max_gap_days: 75 },
      { classes: ['buyer_signal', 'buyer_budget_signal'],                       max_gap_days: 60 },
      { classes: ['buyer_budget_signal', 'enterprise_signal', 'product_signal'], max_gap_days: 90 },
    ],
    predicted_next_moves: [
      'select_implementation_partner',
      'begin_deployment',
  ...
  // Story: Restructuring / cuts → bridge → strategic sale
  {
    id:          'restructure_bridge_sale',
    label:       'Restructuring → bridge → sale',
    description:
      'Efficiency and distress, sustained pressure, bridge-style financing, then exit / acquisition signals',
    type:        'exit',
    sequence: [
      { classes: ['efficiency_signal', 'distress_signal'],                      max_gap_days: 90 },
      { classes: ['efficiency_signal', 'distress_signal'],                      max_gap_days: 75 },
      { classes: ['fundraising_signal', 'distress_signal'],                     max_gap_days: 90 },
      { classes: ['acquisition_signal', 'exit_signal'],                         max_gap_days: 120 },
    ],
    predicted_next_moves: [
      'close_bridge_or_convertible',
      'run_sale_process',
      'enter_exclusivity',
      'close_transaction',
    ],
  ...
  // Story: Hiring → footprint / geo → scale → growth-round financing
  {
    id:          'hiring_office_expansion_series_b',
    label:       'Hiring → offices → expansion → growth round',
    description:
      'Team growth, geographic or office footprint expansion, commercial scale, then growth-stage financing',
    type:        'expansion',
    sequence: [
      { classes: ['hiring_signal'],                                              max_gap_days: 90 },
      { classes: ['expansion_signal'],                                          max_gap_days: 90 },
      { classes: ['expansion_signal', 'enterprise_signal', 'revenue_signal'],   max_gap_days: 120 },
      { classes: ['fundraising_signal', 'investor_interest_signal'],            max_gap_days: 120 },
    ],
    predicted_next_moves: [
      'close_series_b',
      'hire_regional_leaders',
      'open_additional_offices',
      'raise_growth_capital',
    ],
    stage_exit: 'global_commercial_scale',
    who_cares: { investors: true, vendors: true, partners: true, recruiters: true },
...
const STAGE_SIGNAL_MAP = [
  ...
  { stage: 'fundraising_active', requires: ['fundraising_signal', 'investor_interest_signal', 'diligence_signal'], minCount: 1 },
...
const SIGNAL_NEXT_MOVE_MAP = {
  ...
  gtm_hiring_signal:         ['build_pipeline', 'close_enterprise_deals', 'raise_growth_capital'],
  engineering_hiring_signal: ['ship_roadmap', 'harden_platform', 'scale_infra'],
  diligence_signal:          ['sign_term_sheet', 'close_round', 'public_announcement'],
...
function getDominantType(matchedPatterns, dominantSignal) {
  ...
  // Exhaustive map — every signal class in signalOntology.js must appear here.
  // Missing entries cause entities with that dominant signal to get 'unknown' trajectory.
  const TYPE_MAP = {
    fundraising_signal:        'fundraising',
    investor_interest_signal:  'fundraising',
    diligence_signal:          'fundraising',
    investor_rejection_signal: 'fundraising',
    exploratory_signal:        'fundraising',   // hedged intent = early-stage fundraising posture
    exit_signal:               'exit',
    acquisition_signal:        'exit',
    distress_signal:           'distress',
    efficiency_signal:         'distress',
    ...
    market_position_signal:    'expansion',
    partnership_signal:        'expansion',
    hiring_signal:             'growth',
    gtm_hiring_signal:         'growth',
    engineering_hiring_signal: 'growth',
    growth_signal:             'growth',
    ...
    regulatory_signal:         'product',
    infrastructure_signal:     'product',
    grant_signal:              'product',
    patent_signal:             'product',
    university_signal:         'product',
    // Catch-all: negated and unclassified signals indicate watchful waiting
    negated_signal:            'unknown',   // explicit — negated intent is genuinely ambiguous
...
function detectAnomalies(history, windowDays = 90) {
  ...
  const hasInvestorSignal = recent.some(e =>
    ['investor_interest_signal', 'diligence_signal', 'fundraising_signal'].includes(e.signal?.primary_signal)
  );
...
function buildTrajectory(signalHistory, options = {}) {
  ...
  // Prediction string
  // Priority: (1) matched pattern description, (2) velocity-based narrative,
  // (3) trajectory-class narrative, (4) generic signal summary.
  const prediction = primary_pattern?.pattern?.description
    ? primary_pattern.pattern.description
    : (() => {
        const vel  = velocity_score;
        const accel = acceleration;
        const traj  = dominant_trajectory;

        // Velocity + acceleration narratives
        if (accel === 'accelerating' && vel >= 0.60)
          return 'Signal velocity accelerating — company activity is increasing rapidly.';
        if (accel === 'accelerating' && vel >= 0.30)
          return 'Activity momentum building — early acceleration observed across signals.';
        if (accel === 'decelerating' && vel <= 0.20)
          return 'Signal velocity declining — recent activity is slowing down.';

        // Trajectory-class narratives
        const TRAJ_NARRATIVES = {
          fundraising:   'Company shows fundraising trajectory — signals consistent with capital-raise activity.',
          product:       'Product-led signals dominating — company appears to be in an active build or launch phase.',
          growth:        'Growth trajectory detected — expansion and scaling signals present.',
          exit:          'Exit-oriented signals present — company may be exploring strategic alternatives or a sale.',
          distress:      'Distress signals detected — potential runway, restructuring, or bridge capital needs.',
          expansion:     'Expansion trajectory — signals indicate geographic or market expansion underway.',
          buying:        'Buying-mode signals — company appears to be evaluating or procuring new solutions.',
          repositioning: 'Strategic repositioning signals — company may be pivoting product or market focus.',
          unknown:       'Insufficient signal history to establish a clear trajectory — more data needed.',
        };
        if (TRAJ_NARRATIVES[traj]) return TRAJ_NARRATIVES[traj];

        return 'Signal activity detected — trajectory classification in progress.';
      })();
