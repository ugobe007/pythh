...
function founderCtaPrimaryUrl(encodedUrl) {
  return `https://pythh.ai/activate${encodedUrl ? `?startup=${encodedUrl}` : ""}`;
}

function founderCtaText({ encodedUrl = "" } = {}) {
  const primary = founderCtaPrimaryUrl(encodedUrl);
