...
"These [N] investors have recent deals and public writing that overlap with what [startup] is building. Each row says why. Want to walk through the top few?"

Plain English only. Never academic phrasing (no 'sector-label match', 'back companies', 'fit those patterns').
...
/**
 * Opening — plain English, concrete, short.
 */
function founderOpening({ greeting, startupName, count }) {
  const name = startupName || "your startup";
  return (
    `${greeting} These <strong style="color:#94a3b8;">${count}</strong> investors have recent deals and public writing ` +
    `that overlap with what <strong style="color:#94a3b8;">${name}</strong> is building.\n\n` +
    `Each row says why.\n\n` +
...
function founderOpeningText({ greeting, startupName, count }) {
  const name = startupName || "your startup";
  return (
    `${greeting} These ${count} investors have recent deals and public writing that overlap with what ${name} is building.\n\n` +
    `Each row says why.\n\n` +
    `Want to walk through the top few?`
  );
}
...
function founderAlignmentTagline() {
  return "I don't search by sector. I look at what investors have been doing lately.";
}
