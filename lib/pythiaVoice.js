/**
 * Peter — investment analyst behind Pythh outreach.
 * Scouts startups that match investor thesis. Calm, analytical, concise.
 *
...
3. What's the next logical step? (never "Book a demo" — "Take a look", "See whether this aligns", "Are any of these picks worth a discussion?")
...
VC outreach pattern:
"Hi [name], my name is Peter. I am an investment analyst for Pythh.AI. I scout for startups that match investor thesis for review and consideration. One startup, [startup] fits [firm]'s thesis and narrative that may interest you. I included a short list of curated startups that match your thesis for review — see below. Let me know if any of these look interesting and worth your attention. We use 24 algorithms to identify possible candidates for investment including thesis, traction, timing and team."
...
function vcSubject({ firm, emailType, count }) {
  if (emailType === "personal") {
    return `${count} startups that match your thesis`;
  }
  return `${count} startups that match ${firm || "your firm"}`;
}

function vcHeadline({ count }) {
  return `${count} curated startups for review`;
}

function vcOpening({ greeting, firm, isPersonal, featuredStartup, marketObservation }) {
  const firmLabel = firm || "your firm";
  const thesisOwner = isPersonal ? "your" : `${firmLabel}'s`;
  const featured = String(featuredStartup || "").trim();

  let text =
    `${greeting} my name is Peter. I am an investment analyst for Pythh.AI. ` +
    `I scout for startups that match investor thesis for review and consideration. `;

  if (featured) {
    text += `One startup, ${featured} fits ${firmLabel}'s thesis and narrative that may interest you. `;
  } else if (marketObservation) {
    text += `${String(marketObservation).trim()} `;
  }

  text +=
    `I included a short list of curated startups that match ${thesisOwner} thesis for review — see below. ` +
    `Let me know if any of these look interesting and worth your attention.`;
  return text;
}

function vcFootnote() {
  return "Each row includes a short note on why it may fit — I could be wrong on any of them.";
}

function vcMethodology() {
  return (
...
function vcCtaBody() {
  return (
    "See the full list at pythh.ai, or connect Pythh to Claude or Cursor " +
    "if you prefer to review these startups in plain English."
  );
}
...
function vcEmailSignoff() {
  return `${OUTREACH_AGENT_NAME} · Venture Analyst · Pythh · pythh.ai/newsletter`;
}
