...
function founderHeadline({ startupName, count }) {
  ...
  return `${count} investors surfaced for ${name}`;
}

/**
 * Opening — Observation → Reason → Evidence → Question (keep short).
 */
function founderOpening({ greeting, startupName, count }) {
  const name = startupName || "your startup";
  return (
    `${greeting} I noticed several investors whose recent writing and portfolio activity appear to align with ` +
    `<strong style="color:#94a3b8;">${name}</strong> — not by industry label, but by conviction signals: ` +
    `what partners have been discussing, where they've been deploying, and how you fit those patterns today.\n\n` +
    `These <strong style="color:#94a3b8;">${count}</strong> surfaced as unusually strong fits. Each row explains why.\n\n` +
    `Would it be useful to walk through the top few?`
  );
}
...
function founderOpeningText({ greeting, startupName, count }) {
  const name = startupName || "your startup";
  return (
    `${greeting} I noticed several investors whose recent writing and portfolio activity appear to align with ${name} — ` +
    `not by industry label, but by conviction signals: what partners have been discussing, where they've been deploying, ` +
    `and how you fit those patterns today.\n\n` +
    `These ${count} surfaced as unusually strong fits. Each row explains why.\n\n` +
    `Would it be useful to walk through the top few?`
  );
}
