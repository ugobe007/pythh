...
const PETER_CONVICTION_PHILOSOPHY =
  "Most people search for investors by category. I look for conviction — what investors are signaling they care about today, not what they funded years ago.";
...
Voice:
- Calm, concise, respectful of time. Dry humor only — rare, intelligent, never about fundraising.
...
Founder outreach pattern:
"I noticed [observation about investor alignment]. The strongest matches reflected recent conviction signals — not industry labels alone. [Evidence]. Would it be useful to walk through the top few and why they surfaced?"
...
function founderSubject({ startupName, count }) {
  ...
  return `${count} investors worth exploring for ${label}`;
}

function founderHeadline({ startupName, count }) {
  return `${count} investors — conviction signals, not category labels`;
}

/**
 * Opening — Observation → Reason → Evidence → Question.
 */
function founderOpening({ greeting, startupName, count }) {
  const name = startupName || "your startup";
  return (
    `${greeting} I noticed several investors whose recent writing and portfolio activity appear to align closely with ` +
    `<strong style="color:#94a3b8;">${name}</strong>'s direction.\n\n` +
    `The strongest matches weren't based on industry labels alone. They reflected recent conviction signals — ` +
    `what partners have been discussing publicly, where they've been investing, and how your company fits those patterns today.\n\n` +
    `I ran <strong style="color:#94a3b8;">${name}</strong> against 6,000+ investor profiles. These ` +
    `<strong style="color:#94a3b8;">${count}</strong> surfaced as unusually strong fits based on recent investment behavior. ` +
    `Each row below explains why — not just who to contact.\n\n` +
    `Would it be useful to walk through the top few matches and why they surfaced?`
  );
}
...
function founderOpeningText({ greeting, startupName, count }) {
  const name = startupName || "your startup";
  return (
    `${greeting} I noticed several investors whose recent writing and portfolio activity appear to align closely with ${name}'s direction.\n\n` +
    `The strongest matches weren't based on industry labels alone. They reflected recent conviction signals — ` +
    `what partners have been discussing publicly, where they've been investing, and how your company fits those patterns today.\n\n` +
    `I ran ${name} against 6,000+ investor profiles. These ${count} surfaced as unusually strong fits based on recent investment behavior. ` +
    `Each row below explains why — not just who to contact.\n\n` +
    `Would it be useful to walk through the top few matches and why they surfaced?`
  );
}
...
function founderAlignmentTagline() {
  return PETER_CONVICTION_PHILOSOPHY;
}

function founderTableLabel() {
  return "INVESTOR · FIT SIGNAL · WHY THEY SURFACED";
}
...
function founderFootnote() {
  return "I could be wrong on any row — reply if something looks off. Alignment matters more than volume.";
}
...
function defaultMatchReason() {
  return "Appears to align with recent portfolio activity — sector and stage overlap suggest worth exploring.";
}
