...
function classifyEntityTrack(name) {
  ...
  /** Public parents / holding companies scraped as one-word "startups" (Alphabet = Google parent). */
  const SINGLE_TOKEN_PUBLIC_PARENT = new Set([
    'alphabet', // Alphabet Inc. (GOOGL); Alphabet Ventures → GV via investor templates
  ]);

  /** Solo VC brand tokens — "Andreessen" / "Sequoia" / "a16z" scraped as startups. */
  const SINGLE_TOKEN_VC_BRAND = new Set([
    // Only unambiguous standalone VC brands from SLOT_FIRST_VC_BRAND
    // Omit ambiguous tokens like 'tiger', 'peak', 'vista', 'bravo', 'francisco', 'thoma'
    // that require multi-word context to signal VC funds
    'sequoia', 'andreessen', 'horowitz', 'kleiner', 'perkins',
    'greylock', 'bessemer', 'felicis', 'coatue', 'softbank',
    'warburg', 'pincus', 'kkr', 'blackstone', 'carlyle', 'apax',
    'dcvc', 'capitalg', 'base10',
    // Unambiguous accelerator/fund abbreviations
    'a16z',
    'yc',
    'ycombinator',
  ]);
  ...
  if (wc === 1 && !hasCamelCase) {
    ...
    if (SINGLE_TOKEN_PUBLIC_PARENT.has(lower)) {
      return { track: 'descriptor', confidence: 'high', reason: `[${lower}:public-parent-holding]→megacorp-not-startup` };
    }
    if (SINGLE_TOKEN_VC_BRAND.has(lower)) {
      return { track: 'investor', confidence: 'high', reason: `[${lower}:solo-vc-brand]→investor` };
    }
  }
