...
function hasDbConfigOverrides(config) {
  return config && typeof config === 'object' && Object.keys(config).length > 0;
}

/**
 * DB `version` column can lag behind code defaults when config JSON is empty.
 * Effective version: saved config.version → DB column → code default.
 */
function resolveConfigVersion(dbConfig, dbVersionColumn) {
  if (hasDbConfigOverrides(dbConfig)) {
    if (dbConfig.version) return String(dbConfig.version);
    if (dbVersionColumn) return String(dbVersionColumn);
  }
  return DEFAULT_SIGNAL_WEIGHT_CONFIG.version;
}
...
async function loadSignalWeightConfig(supabase) {
  ...
  try {
    ...
    const merged = mergeSignalWeightConfig(dbConfig);
    merged.version = resolveConfigVersion(dbConfig, data?.version);
...
/**
 * Same as loadSignalWeightConfig but includes DB vs effective version metadata for logging/UI.
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 */
async function loadSignalWeightConfigWithMeta(supabase) {
  const config = await loadSignalWeightConfig(supabase);
  try {
    ...
    return {
      config,
      meta: {
        dbVersion: data?.version ?? null,
        effectiveVersion: config.version,
        hasAdminOverrides: hasDbConfigOverrides(dbConfig),
      },
    };
  } catch {
    return {
  ...
  const { error: upErr } = await supabase.from('signal_weight_config').upsert(
    {
      id: 1,
      config: payload,
      version: payload.version || DEFAULT_SIGNAL_WEIGHT_CONFIG.version,
      updated_at: now,
...
module.exports = {
  ...
  loadSignalWeightConfigWithMeta,
