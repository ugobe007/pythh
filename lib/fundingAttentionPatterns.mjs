...
const INSTITUTIONAL_NAME_RE =
  /\b(ventures?|capital|partners?|fund|group|holdings|investments?|investors?|associates|accelerator|incubator|family office|tech cent(?:er|re))\b/i;

const INSTITUTIONAL_ALIASES = new Set([
  'plug and play',
  'plug and play tech center',
  'qed',
  'qed investors',
  'bpifrance',
  'bpifrance amorcage industriel',
]);
...
function nameLooksLikeFirm(investor = {}) {
  ...
  if (INSTITUTIONAL_ALIASES.has(name)) return true;
  if (INSTITUTIONAL_ALIASES.has(firm) && !looksLikePersonName(investor.name) && investor.is_individual !== true) return true;
  // Suffix on the display name wins even when the weak person heuristic misfires
  // ("QED Investors", "Plug and Play Tech Center").
  if (INSTITUTIONAL_NAME_RE.test(String(investor.name || ''))) return true;
  if (
    INSTITUTIONAL_NAME_RE.test(String(investor.firm || ''))
    && !looksLikePersonName(investor.name)
    && investor.is_individual !== true
  ) {
    return true;
  }
  return false;
...
export function classifyCapitalRole(investor = {}) {
  ...
  if (nameLooksLikeFirm(investor) && investor.is_individual !== true) {
    return {
      role: 'firm',
  ...
  const hasFounderLanguage = founder.signals.includes('founder_exit_language') 
    || founder.signals.includes('known_operator_founder');
  if (knownFounder || (founder.isOperatorFounder && hasFounderLanguage && person)) {
    return {
      role: 'founder_angel',
      why: knownFounder
...
export function detectFollowTheLead(startupEvents = []) {
  ...
  for (const event of sorted) {
    ...
    const gapMs = Date.parse(eventTime(event) || 0) - Date.parse(leader.announced_at || 0);
    if (!Number.isFinite(gapMs) || gapMs < 2 * 86_400_000) continue;
    ...
    if (followed.length) {
      followers.push({
        ...
        investor_ids: followed.map((row) => row.id).filter(Boolean),
      });
    }
  }
...
export function buildPatternReport({ events = [], investorsById = new Map() } = {}) {
  ...
  for (const event of events) {
    ...
    const startupKey = event.startup_id || (norm(event.startup_name_raw) ? `name:${norm(event.startup_name_raw)}` : null);
    if (startupKey) {
    ...
    for (const person of roster) {
      ...
      if (vehicle.vehicle === 'personal_angel') {
        sidecar.push({
          investor: person.name || person.firm,
          firm: person.firm || null,
          role: vehicle.role,
