...
function selectTopMatchesByFirm(scoredRows, investorById, limit, opts = {}) {
  ...
  const firmKeyOf = (row) => {
    ...
    return canonicalFirmKey(inv);
  };

  const tryAdd = (row, { evictSameFirm = false } = {}) => {
    const id = investorIdOf(row, getId);
    ...
    if (seenFirms.has(firmKey)) {
      if (!evictSameFirm) return false;
      const collide = selected.findIndex((existing) => firmKeyOf(existing) === firmKey);
      if (collide < 0) return false;
      const collideId = investorIdOf(selected[collide], getId);
      if (force.has(collideId)) return false;
      const evicted = selected.splice(collide, 1)[0];
      seenIds.delete(investorIdOf(evicted, getId));
      seenFirms.delete(firmKey);
    }
