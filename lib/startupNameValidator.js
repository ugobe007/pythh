...
const NON_COMPANY_EXACT_NAMES = new Set(
  ...
  'e-commerce', 'e commerce', 'over 100',
  ...
  'cherokee alabama', 'cherokee nation',
    ...
    // Alphabet = Google's public parent; Alphabet Ventures → GV (Google Ventures)
    ...
    'google parent alphabet',
    // Solo VC brands scraped as startups (lead to junk matches)
    'andreessen',
    'a16z',
    'sequoia',
    'greylock',
...
const GENERIC_SINGLE_WORDS = new Set([
  ...
  // Parser fragments from funding-headline extraction (single-token junk)
  'that','daddy','company','famous','boil','base','urban','illness','sources',
...
const JUNK_PATTERNS = [
  ...
  /\bDaily\s+Open\b/i,
  /^Over\s+\d+\b/i,
  // Megacorp parent legal-name / possessive fragments (Alphabet Inc.'s …)
  /^Alphabet\s+Inc\.?'?s?\b/i,
  /\bAlphabet\s+Inc\.?'?s?\s*$/i,
