...
function isAcquisition(text) {
  return /\bacquire[sd]?\b|\bacquired by\b|\bacquisition of\b|\b(?:closes|closed)\b[^.]{0,48}\bacquisition\b|\bnow a part of\b/i.test(text);
}

function isNewFund(text) {
  return /\blaunch(?:es|ed)?\b[^.]{0,40}\bfund\b|\b(?:venture fund|new fund|interval fund)\b/i.test(text);
}
...
function acquisitionParties(text) {
  const patterns = [
    {
      re: /\b([A-Z][\w.&'-]+) (?:officially |has |will )?(?:closes|closed) (?:its |a )?([A-Z][\w.&'-]+) acquisition\b/,
      company: 2,
      counterparty: 1,
    },
    {
