...
function classifyCapitalProvider(name, evidencePhrase = '') {
  ...
  // CVC: must be in name OR (corporate/strategic in phrase + corp suffix in name)
  if (CVC_RE.test(n) || (CVC_RE.test(phrase) && /\b(?:inc|corp|technologies|motors|systems)\b/i.test(n)) || (STRATEGIC_PHRASE_RE.test(phrase) && /\b(?:inc|corp|technologies|motors|systems)\b/i.test(n))) {
  ...
  // Strategic: phrase mentions strategic BUT name doesn't look like VC and isn't attributable
  const escaped = n.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const attributedStrategic = new RegExp(
    `(?:^|\\b)${escaped}\\s*[,\\-–—:]?\\s*(?:(?:is|are)\\s+)?(?:a\\s+|an\\s+)?strategic\\s+(?:investment|investor)\\b`,
    'i',
  );
  if (STRATEGIC_PHRASE_RE.test(phrase) && !VC_SUFFIX_RE.test(n) && attributedStrategic.test(phrase)) {
  ...
  // Debt: must be in name OR (debt phrase attributed to this name)
  const attributedDebt = new RegExp(
    `(?:^|\\b)${escaped}\\s*[,\\-–—:]?\\s*(?:(?:provides?d?|offering)\\s+)?(?:a\\s+|an\\s+)?(?:venture\\s+debt|growth\\s+credit|debt\\s+facility|term\\s+loan)\\b|\\b(?:venture\\s+debt|growth\\s+credit|debt\\s+facility)\\s+(?:from\\s+)?${escaped}\\b`,
    'i',
  );
  if (DEBT_RE.test(n) || /\bsilicon\s+valley\s+bank\b/i.test(n) || (DEBT_RE.test(phrase) && attributedDebt.test(phrase))) {
  ...
  // Accelerator: must be in name OR (accelerator phrase attributed to this name)
  const attributedAccel = new RegExp(
    `(?:^|\\b)${escaped}\\s*[,\\-–—:]?\\s*(?:(?:is|are)\\s+)?(?:a\\s+|an\\s+)?(?:accelerator|incubator|studio)\\b|\\b(?:backed|supported)\\s+by\\s+(?:y\\s*combinator|\\byc\\b|techstars|500\\s*global|antler|${escaped})\\b`,
    'i',
  );
  if (ACCEL_RE.test(n) || (ACCEL_RE.test(phrase) && attributedAccel.test(phrase))) {
