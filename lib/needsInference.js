...
const SIGNAL_NEED_MAP = {
  ...
  investor_rejection_signal: [
    { need: 'series_a_capital',         weight: 0.42, condition: null },
    { need: 'banker_advisor',           weight: 0.38, condition: null },
  ],
  product_signal: [
  ...
  growth_signal: [
    { need: 'growth_capital',           weight: 0.70, condition: null },
    { need: 'revops_tools',             weight: 0.60, condition: null },
    { need: 'lead_generation',          weight: 0.55, condition: null },
  ],
};

// INFERENCE_MAP `likely_need` strings (from signalOntology) → canonical NEED_CLASSES
const STRATEGIC_NEED_HINT_TO_CANONICAL = {
  legal:                    { need: 'banker_advisor',           weight: 0.45 },
  accounting:               { need: 'compliance_tools',         weight: 0.40 },
  cap_table_management:     { need: 'series_a_capital',         weight: 0.35 },
  CRM:                      { need: 'revops_tools',             weight: 0.55 },
  investor_relations:       { need: 'lead_generation',          weight: 0.40 },
  localization:             { need: 'localization_support',     weight: 0.55 },
  HR:                       { need: 'executive_search',         weight: 0.45 },
  regional_marketing:       { need: 'lead_generation',          weight: 0.42 },
  'M&A_advisory':          { need: 'banker_advisor',           weight: 0.60 },
  integration_tools:        { need: 'implementation_support',   weight: 0.50 },
  compliance_tools:       { need: 'compliance_tools',         weight: 0.55 },
  QA:                       { need: 'compliance_tools',         weight: 0.35 },
  regulatory_consultant:    { need: 'compliance_tools',         weight: 0.50 },
  automation:               { need: 'automation_vendor',        weight: 0.50 },
  robotics:                 { need: 'robotics_vendor',          weight: 0.50 },
  software:                 { need: 'pilot_partner',            weight: 0.40 },
  vendor:                   { need: 'procurement_support',      weight: 0.45 },
  platform:                 { need: 'pilot_partner',            weight: 0.40 },
  implementation_partner:   { need: 'implementation_support',   weight: 0.55 },
  debt_financing:           { need: 'bridge_capital',           weight: 0.55 },
  cost_reduction_tools:     { need: 'turnaround_support',       weight: 0.45 },
  turnaround_advisory:      { need: 'turnaround_support',       weight: 0.55 },
  restructuring:            { need: 'turnaround_support',       weight: 0.50 },
  analytics:                { need: 'data_tools',               weight: 0.50 },
  AI:                       { need: 'data_tools',               weight: 0.45 },
  pitch_deck_tools:         { need: 'lead_generation',          weight: 0.35 },
  data_room:                { need: 'series_a_capital',         weight: 0.30 },
  finance_tools:            { need: 'revops_tools',             weight: 0.45 },
  revenue_ops:              { need: 'revops_tools',             weight: 0.50 },
  office_space:             { need: 'strategic_partner',        weight: 0.25 },
};
 ...
 * @param {object}                 [options.enrichment]            — optional startup_uploads.extracted_data subset:
 *                                                                    `{ ontology_inference, market_signals }` from news enrichment
...
function inferNeeds(signalHistory, trajectory, options = {}) {
  ...
  const { colloquialToGrammarPrimarySignal } = require('./ontologyCrosswalk');
  ...
  // ── Step 1b: Enrichment grammar (extracted_data.ontology_inference) ─────
  if (enrichment?.ontology_inference?.signal_classes?.length) {
    for (const sc of enrichment.ontology_inference.signal_classes) {
      const cls = sc.signal_class;
      if (!cls) continue;
      const conf = typeof sc.best_certainty === 'number' ? sc.best_certainty : 0.55;
      const mappings = SIGNAL_NEED_MAP[cls] || [];
      for (const { need, weight } of mappings) {
        bump(need, weight * conf, 'signal', cls);
      }
  ...
  // ── Step 1c: Strategic need hints (INFERENCE_MAP likely_need strings) ───
  for (const key of enrichment?.ontology_inference?.inferred_strategic_needs || []) {
    const row = STRATEGIC_NEED_HINT_TO_CANONICAL[key];
    if (row) bump(row.need, row.weight * 0.5, 'enrichment', `strategic:${key}`);
  }

  // ── Step 1d: Colloquial market_signals → grammar via crosswalk ───────────
  const ms = enrichment?.market_signals;
  const primaryColloquial =
    typeof ms?.primarySignal === 'string'
      ? ms.primarySignal
      : ms?.primarySignal?.signal;
  if (primaryColloquial) {
    const grammar = colloquialToGrammarPrimarySignal(primaryColloquial);
    if (grammar) {
      const sigMatch = (ms.signals || []).find((s) => s.signal === primaryColloquial);
      const conf = typeof sigMatch?.score === 'number' ? Math.min(1, sigMatch.score) : 0.55;
      const mappings = SIGNAL_NEED_MAP[grammar] || [];
      for (const { need, weight } of mappings) {
        bump(need, weight * conf * 0.85, 'signal', grammar);
      }
    }
  }
...
module.exports = {
  ...
  SIGNAL_NEED_MAP,
  TRAJECTORY_NEED_MAP,
  STRATEGIC_NEED_HINT_TO_CANONICAL,
};
