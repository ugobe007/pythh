...
function scoreSignalAlignment(candidate, trajectory, needs) {
  ...
  // When a candidate has no declared preferences we grant a neutral score
  // that's slightly above 0.40 so under-specified candidates don't
  // systematically drag down every match — but clearly below a genuine match.
  if (supported.size === 0) return 0.50;
 ...
 * @param {number}              [options.min_score=0.45]    — filter threshold
...
function rankMatches(entity, trajectory, needs, candidates, options = {}) {
  const minScore   = options.min_score   ?? 0.45;
