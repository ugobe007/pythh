...
const ACTION_MAP = [
  ...
  ['(had (planned|intended|hoped|expected|aimed) to (raise|launch|expand|hire|acquire|close|build))',
    { signal_class: 'exploratory_signal', base_certainty: 0.30, action_tag: 'action_exploring',
      ...
      gates: ['fundraising_signal', 'hiring_signal', 'gtm_hiring_signal', 'engineering_hiring_signal', 'diligence_signal', 'acquisition_signal', 'expansion_signal', 'product_signal'] }],
  ['(was (hoping|planning|intending|expecting) to (raise|launch|expand|hire|close))',
    { signal_class: 'exploratory_signal', base_certainty: 0.30, action_tag: 'action_exploring',
      ...
      gates: ['fundraising_signal', 'hiring_signal', 'gtm_hiring_signal', 'engineering_hiring_signal', 'product_signal', 'expansion_signal'] }],
  ...
  // ── Formal diligence / data room (distinct from generic “interest” or “raising”) ─
  ['(granted|grant(ing)?|provided?|providing|opened?|opening|access to) (the |a )?(virtual )?data.?room',
    { signal_class: 'diligence_signal',          base_certainty: 0.96, action_tag: 'action_investor_diligence', meaning: 'data room opened — late-stage diligence' }],
  ['requested? (the )?data.?room',
    { signal_class: 'diligence_signal',          base_certainty: 0.95, action_tag: 'action_investor_diligence', meaning: 'investor in / requesting data room' }],
  ['\\b(vdr|virtual data room)\\b',
    { signal_class: 'diligence_signal',          base_certainty: 0.92, action_tag: 'action_investor_diligence', meaning: 'VDR activity' }],
  ['(in |under |entering |conduct(ing|ed) |complet(ing|ed) )?(legal |financial |buy.?side )?due diligence',
    { signal_class: 'diligence_signal',          base_certainty: 0.90, action_tag: 'action_investor_diligence', meaning: 'formal due diligence' }],
  ['diligence (is |was )?(underway|ongoing|in progress|nearly complete|wrapping up|complete)',
    { signal_class: 'diligence_signal',          base_certainty: 0.88, action_tag: 'action_investor_diligence', meaning: 'diligence process update' }],
  ...
  // ── Typed hiring — must precede generic hire / recruit patterns ─────────────
  ['(hir(es|ing|ed)|recruit(s|ing|ed)|search(ing|es|ed) for|looking for) .{0,50}(vp of sales|vp sales|head of sales|chief revenue officer|\\bcro\\b|sales director|account executive|\\baes?\\b|\\bsdrs?\\b|\\bbdrs?\\b|business development|\\bbd\\b|go-?to-?market|\\bgtm\\b|revenue leadership|demand gen|growth marketer)',
    { signal_class: 'gtm_hiring_signal',         base_certainty: 0.88, action_tag: 'action_gtm_hiring',         meaning: 'GTM or sales-led hiring' }],
  ['(sales|gtm|revenue|commercial|marketing) (team )?(is |are )?(hiring|growing|scaling|expanding|building)',
    { signal_class: 'gtm_hiring_signal',         base_certainty: 0.85, action_tag: 'action_gtm_hiring',         meaning: 'commercial team expansion' }],
  ['hir(es|ing|ed) (a |an )?(vp of marketing|chief marketing officer|\\bcmo\\b|head of marketing)',
    { signal_class: 'gtm_hiring_signal',         base_certainty: 0.90, action_tag: 'action_gtm_hiring',         meaning: 'marketing leadership hire' }],
  ['(hir(es|ing|ed)|recruit(s|ing|ed)|search(ing|es|ed) for|looking for) .{0,50}(software engineer|engineers?|developers?|engineering manager|vp of engineering|head of engineering|chief technology officer|\\bcto\\b|platform engineer|\\bsre\\b|devops|machine learning engineer|\\bmle\\b|data engineer|security engineer|infra(structure)?)',
    { signal_class: 'engineering_hiring_signal', base_certainty: 0.88, action_tag: 'action_engineering_hiring', meaning: 'engineering or product-tech hiring' }],
  ['(engineering|platform|product engineering) (team )?(is |are )?(hiring|growing|scaling)',
    { signal_class: 'engineering_hiring_signal', base_certainty: 0.85, action_tag: 'action_engineering_hiring', meaning: 'engineering team expansion' }],
  ['named? .{0,25}(chief technology officer|\\bcto\\b|vp of engineering|head of engineering)',
    { signal_class: 'engineering_hiring_signal', base_certainty: 0.95, action_tag: 'action_leadership_hire',   meaning: 'engineering leadership appointment' }],
  ['named? .{0,25}(chief marketing officer|\\bcmo\\b|chief revenue officer|\\bcro\\b|vp of sales|head of sales)',
    { signal_class: 'gtm_hiring_signal',         base_certainty: 0.95, action_tag: 'action_leadership_hire',   meaning: 'GTM leadership appointment' }],

  // ── Hiring / team ─────────────────────────────────────────────────────────
  ['appoint(ed|ing)',
    { signal_class: 'hiring_signal',             base_certainty: 0.90, action_tag: 'action_leadership_hire',  meaning: 'leadership appointment' }],
  ['named? .{0,25}(ceo|coo|cpo|vp |president|head of)',
  ...
  // Headline / wire form: "$5.5 million" instead of "$5.5M"
  ['rais(ed|ing|es|e)\\s+\\$[\\d.,]+\\s+(million|billion|thousand)\\b',
    { signal_class: 'fundraising_signal',        base_certainty: 0.95, action_tag: 'action_raising',          meaning: 'fundraise with spelled-out dollar amount' }],
  ['(has|have)\\s+rais(ed|ing|es|e)\\s+\\$[\\d.,]+(\\s+(million|billion|thousand)\\b|[KkMmBb])',
    { signal_class: 'fundraising_signal',        base_certainty: 0.96, action_tag: 'action_closing_round',      meaning: 'present-perfect financing closed (has raised $X)' }],
  ['\\$[\\d.,]+\\s+(million|billion)\\s+in\\s+(growth\\s+)?capital\\b',
    { signal_class: 'fundraising_signal',        base_certainty: 0.94, action_tag: 'action_closing_round',      meaning: 'growth capital / expansion round (amount in words)' }],
  ['\\b(capital raise|the raise|financing round),?\\s+led by\\b',
    { signal_class: 'fundraising_signal',        base_certainty: 0.90, action_tag: 'action_closing_round',      meaning: 'round led by (named lead / syndicate)' }],
  ['secures?\\s+\\$[\\d.,]+[KkMmBb]',
  ...
  ['(total|cumulative) (raised|funding) (of |: |to (over )?)?\\$[\\d.,]+\\s+(million|billion|thousand)\\b',
    { signal_class: 'fundraising_signal',        base_certainty: 0.91, action_tag: 'action_closing_round',    meaning: 'total funding milestone (spelled-out amount)' }],
  ['bringing\\s+.+?\\s+total funding to (over )?\\$[\\d.,]+(\\s+(million|billion|thousand)\\b|[KkMmBb]\\b)',
    { signal_class: 'fundraising_signal',        base_certainty: 0.93, action_tag: 'action_total_funding_to_date', meaning: 'cumulative funding to date (bringing total to $X)' }],
  ['announc(ed|es|ing)? (the )?close of (a |the )?\\$[\\d.,]+(\\s+(million|billion|thousand)\\b|[KkMmBb])',
    { signal_class: 'fundraising_signal',        base_certainty: 0.97, action_tag: 'action_closing_round',    meaning: 'announced close of financing round (wire)' }],
  ['\\b(a |the )?new round of funding\\b',
    { signal_class: 'fundraising_signal',        base_certainty: 0.88, action_tag: 'action_raising',          meaning: 'new funding round (alongside product or milestone)' }],
  ['alongside (a |the )?new round of funding\\b',
    { signal_class: 'fundraising_signal',        base_certainty: 0.89, action_tag: 'action_raising',          meaning: 'funding alongside launch or announcement' }],
  ...
  ['(kicking off|starting) (a |our |the )?(seed|series|bridge|extension|next )?round',
    { signal_class: 'fundraising_signal',        base_certainty: 0.80, action_tag: 'action_raising',          meaning: 'raise kicking off' }],
  ['(meeting|speaking) with (investors|funds|partners)',
    { signal_class: 'fundraising_signal',        base_certainty: 0.70, action_tag: 'action_raising',          meaning: 'investor conversations underway' }],
  ['(allocation|check size) (is )?(tight|limited|competitive)',
    { signal_class: 'fundraising_signal',        base_certainty: 0.65, action_tag: 'action_raising',          meaning: 'hot round / competitive allocation' }],
  ['(product[- ]market fit|\\bpmf\\b|founder[- ]market fit)',
    { signal_class: 'growth_signal',             base_certainty: 0.80, action_tag: 'action_growing',          meaning: 'PMF / founder-market fit language' }],
  ['(default alive|path to profitability|default.?dead)',
    { signal_class: 'efficiency_signal',         base_certainty: 0.75, action_tag: 'action_efficiency',       meaning: 'capital discipline / survival framing' }],
  ['(burn multiple|magic number|cac payback|ltv.?cac)',
    { signal_class: 'revenue_signal',            base_certainty: 0.75, action_tag: 'action_revenue',          meaning: 'SaaS unit economics shorthand' }],
  ['(north star|okrs?|company[- ]level metrics?)',
    { signal_class: 'efficiency_signal',         base_certainty: 0.55, action_tag: 'action_efficiency',       meaning: 'operating cadence / goals' }],
  ...
  ['send (over |us )?(the |your )?data.?room',
    { signal_class: 'diligence_signal',          base_certainty: 0.95, action_tag: 'action_investor_diligence', meaning: 'data room requested — formal diligence' }],
  ...
  // ── Investor: Deal mechanics & shared shorthand (founders + VCs) ───────────
  ['\\b(safe|post-money|pre-money|cap table|pro[- ]rata|liquidation preference)\\b',
    { signal_class: 'fundraising_signal',        base_certainty: 0.85, action_tag: 'action_raising',          meaning: 'deal structure / terms language' }],
  ['(dry powder|deploy(ing)? capital|reserve(d)? (for )?follow[- ]on)',
    { signal_class: 'fundraising_signal',        base_certainty: 0.70, action_tag: 'action_closing_round',    meaning: 'investor capital deployment' }],
  ['(lead|anchor) (round|check)\\b',
    { signal_class: 'fundraising_signal',        base_certainty: 0.80, action_tag: 'action_closing_round',    meaning: 'lead or anchor on the round' }],
  // Avoid stealing "Lead investor requested the data room" from diligence_signal
  ['(lead|anchor) investor(?!.{0,70}(request|requested|ask(ed|ing)|data.?room|deck|financials))',
    { signal_class: 'fundraising_signal',        base_certainty: 0.78, action_tag: 'action_closing_round',    meaning: 'lead / anchor investor (deal context)' }],
  ['(warm intro|double opt-in|syndicate (lead|member))',
    { signal_class: 'investor_interest_signal',  base_certainty: 0.65, action_tag: 'action_investor_interest', meaning: 'process / intro mechanics' }],
  ['(platform (round|play)|value[- ]add (beyond )?capital)',
    { signal_class: 'investor_interest_signal',  base_certainty: 0.60, action_tag: 'action_investor_interest', meaning: 'VC platform or value-add' }],
  ['(ownership|dilution) (cap|limits?|sensitivity)',
    { signal_class: 'fundraising_signal',        base_certainty: 0.75, action_tag: 'action_raising',          meaning: 'ownership / dilution framing' }],
  ['(milestone[- ]based|tranche) (financing|funding|raise)',
    { signal_class: 'fundraising_signal',        base_certainty: 0.80, action_tag: 'action_raising',          meaning: 'structured milestone financing' }],
  ...
  ['(began|started) working with',
    { signal_class: 'partnership_signal',        base_certainty: 0.82, action_tag: 'action_partnering',       meaning: 'began partnership or collaboration (past inception)' }],
  ['dropped (their|the|its)? ?(investment|stake|backing|position)',
    { signal_class: 'fundraising_signal',        base_certainty: 0.78, action_tag: 'action_exiting',          meaning: 'exited or reduced investment position' }],
  ['decided to build',
    { signal_class: 'product_signal',            base_certainty: 0.78, action_tag: 'action_building',         meaning: 'strategic decision to build in-house (vs buy/partner)' }],
  ...
  // ── Founder culture / psychology (VC thesis: founders ARE the startup) ─────
  ['(couldn.?t be more (excited|proud|grateful)|humbled (by|and)|deeply grateful|honored to (announce|share|lead))',
    { signal_class: 'founder_psychology_signal', base_certainty: 0.82, action_tag: 'action_founder_posture',  meaning: 'positive founder posture — often precedes milestone' }],
  ['(revisiting (our )?roadmap|customers kept asking|listening to (our )?customers|customer feedback (changed|shaped))',
    { signal_class: 'founder_psychology_signal', base_certainty: 0.78, action_tag: 'action_founder_posture',  meaning: 'customer-driven reconsideration — pivot or product insight' }],
  ['(raise the bar|raising the bar|world.?class|best.?in.?class|obsessed with (quality|excellence|details)|insanely great|never settle|relentless(ly)? (pursu|about|focus))',
    { signal_class: 'founder_excellence_signal', base_certainty: 0.80, action_tag: 'action_excellence',       meaning: 'excellence bar — high standards culture' }],
  ['(ship(ped|ping)? fast|move fast|bias for action|rapid iteration|iterate quickly|build in public|dogfooding|default alive)',
    { signal_class: 'founder_cunning_signal',    base_certainty: 0.78, action_tag: 'action_speed',            meaning: 'cunning execution — ship fast, learn fast' }],
  ['(delight(ed|ing)? (our )?customers|customers love (us|what|when)|fanatical(ly)? about customers|customer obsessed|love our users)',
    { signal_class: 'customer_delight_signal',   base_certainty: 0.82, action_tag: 'action_customer_delight', meaning: 'customer delight obsession' }],
  ['(thrilled to welcome|excited to welcome|joined us from|welcome aboard|proud to announce .{0,40} (joining|joined)|world.?class team|magnet for talent|attract(ing|ed)? top talent)',
    { signal_class: 'talent_magnet_signal',      base_certainty: 0.85, action_tag: 'action_talent_inbound',   meaning: 'talent magnet — A-players joining' }],

  // ── Failure learning vs exit (split farewell from comeback) ───────────────
  ['((what we learned|lessons (we )?learned|learned (so much|a lot|from this)) .{0,60}(now|today|next|building|launching|starting|back|again|return))',
    { signal_class: 'failure_learning_signal',   base_certainty: 0.82, action_tag: 'action_failure_learning', meaning: 'failure converted to learning — comeback signal' }],
  ['(failed before|previous startup (failed|didn.?t work)|this time (we|i)|trying again|back at it|second act|building again after)',
    { signal_class: 'failure_learning_signal',   base_certainty: 0.78, action_tag: 'action_failure_learning', meaning: 'prior failure acknowledged with forward motion' }],
  ['(sunsetting|winding down|shutting down|ceasing operations|closing (the )?company|going out of business)',
    { signal_class: 'failure_exit_signal',       base_certainty: 1.00, action_tag: 'action_shutdown',         meaning: 'confirmed shutdown or wind-down' }],
  ['(bittersweet|mixed emotions|emotional day)',
    { signal_class: 'failure_exit_signal',       base_certainty: 0.80, action_tag: 'action_shutdown',         meaning: 'emotional transition — shutdown or exit' }],
  ['on to (the )?next (adventure|chapter|thing)',
    { signal_class: 'failure_exit_signal',       base_certainty: 0.85, action_tag: 'action_shutdown',         meaning: 'departure language — winding down or leaving' }],
  ['(stepping back|stepping down|moving on)(?! .{0,40}(building|launching|starting|new venture))',
    { signal_class: 'failure_exit_signal',       base_certainty: 0.85, action_tag: 'action_shutdown',         meaning: 'founder exiting or stepping back' }],
  ['(this was not an easy|not an easy) decision',
    { signal_class: 'failure_exit_signal',       base_certainty: 0.85, action_tag: 'action_shutdown',         meaning: 'difficult decision language — shutdown or pivot' }],
  ['after (much |a lot of )?(reflection|soul.?searching)(?! .{0,50}(building|launching|excited to))',
    { signal_class: 'failure_exit_signal',       base_certainty: 0.80, action_tag: 'action_shutdown',         meaning: 'post-failure reflection — often precedes announcement' }],
  ['(we learned a lot|incredibly valuable lessons|this has been a (crazy|wild|tough) journey)',
    { signal_class: 'failure_exit_signal',       base_certainty: 0.75, action_tag: 'action_shutdown',         meaning: 'implicit failure acknowledgment — farewell phrasing' }],
  ['(it (has|hasn.?t) been easy|not been easy)',
    { signal_class: 'failure_exit_signal',       base_certainty: 0.78, action_tag: 'action_shutdown',         meaning: 'struggle acknowledgment — difficulty admitted' }],
  ['the honest truth (is|about)',
    { signal_class: 'failure_exit_signal',       base_certainty: 0.75, action_tag: 'action_shutdown',         meaning: 'transparency opener — often precedes hard news' }],

  // ── Distress / shutdown (operational — not founder farewell tone) ─────────
];
...
const TIME_MAP = [
  ...
  [/\bby\s+(20\d{2})\b/i,
    { tag: 'time_long_horizon',  proximity: 0.35 }],
  [/\b(Fall|Spring|Summer|Winter)\s+(20\d{2})\b/i,
    { tag: 'time_season_year',   proximity: 0.72 }],
  [/\b(after (the )?(close|round|launch|hire|partnership|announcement))\b/i,
    { tag: 'time_post_event',    proximity: 0.5 }],
];

// ─── VERB TENSE / ASPECT (linguistic time of the *verb*, not calendar TIME_MAP) ─
// Distinguishes completed events vs present stance vs future projection — e.g. press
// lines: "Sequoia funded X last year" (past) + "it is betting Y will be similar" (present + future).
// Consumed by signalParser → `verb_tense` on each SignalObject; feeds inferred_meanings.
const VERB_TENSE_ASPECT = Object.freeze({
  COMPLETED_PAST:       'completed_past',        // simple past / historical deed
  PRESENT_ANALYTIC:     'present_analytic',     // present: current view, habit, or standing state
  PRESENT_PROGRESSIVE:  'present_progressive', // am/is/are + -ing (in progress)
  PRESENT_PERFECT:      'present_perfect',     // have/has + participle (relevance to now)
  FUTURE_PROJECTION:    'future_projection',   // will / going to / expected to
  EPISTEMIC_PAST:       'epistemic_past',      // was unaware / learned later — information state in the past
  PAST_CONTINUOUS:      'past_continuous',     // was/were + -ing — parallel ongoing action in the past
});

// [ RegExp, { aspect, weight, meaning } ] — weight used when deduping same aspect (keep max).
const VERB_TENSE_MAP = [
  // ── Future / modal forward (specific phrases before generic "will") ────────
  [/\bwill be .{0,40}(similar|analog|parallel|same|comparable)\b/i,
    { aspect: VERB_TENSE_ASPECT.FUTURE_PROJECTION, weight: 0.90, meaning: 'future projection — analogy or classification ahead' }],
  [/\b(will fund|will lead|will invest|will co-?lead|will participate|will back|will write|will announce)\b/i,
    { aspect: VERB_TENSE_ASPECT.FUTURE_PROJECTION, weight: 0.95, meaning: 'simple future — explicit forward commitment' }],
  [/\b(funding|capital|the round|proceeds|investment|new funding) will (support|fund|fuel|drive|accelerate|enable)\b/i,
    { aspect: VERB_TENSE_ASPECT.FUTURE_PROJECTION, weight: 0.88, meaning: 'future use-of-proceeds / deployment (press release)' }],
  [/\b(plan to|plans to) (launch|open|scale|expand|build)\b/i,
    { aspect: VERB_TENSE_ASPECT.FUTURE_PROJECTION, weight: 0.86, meaning: 'near-future operational plan' }],
  [/\b(going to (fund|invest|lead|raise|close|announce|acquire|merge))\b/i,
    { aspect: VERB_TENSE_ASPECT.FUTURE_PROJECTION, weight: 0.88, meaning: 'going to + verb — near-term intent' }],
  [/\b(expected to|set to|scheduled to|slated to) (fund|invest|raise|close|announce|ipo)\b/i,
    { aspect: VERB_TENSE_ASPECT.FUTURE_PROJECTION, weight: 0.82, meaning: 'scheduled / expected future event' }],
  [/\bpoised to (fund|invest|raise|close|go public|list)\b/i,
    { aspect: VERB_TENSE_ASPECT.FUTURE_PROJECTION, weight: 0.80, meaning: 'poised to — forward posture' }],

  // ── Present perfect / relevance of a past act to now ──────────────────────
  [/\b(has|have) (already )?(funded|led|co-?led|backed|invested in|closed|announced|acquired)\b/i,
    { aspect: VERB_TENSE_ASPECT.PRESENT_PERFECT, weight: 0.92, meaning: 'present perfect — deed with present relevance' }],
  [/\b(has|have) raised \$[\d.,]+/i,
    { aspect: VERB_TENSE_ASPECT.PRESENT_PERFECT, weight: 0.94, meaning: 'present perfect — financing closed (has raised $X)' }],
  [/\b(has|have) been (funding|investing|backing|building|scaling)\b/i,
    { aspect: VERB_TENSE_ASPECT.PRESENT_PERFECT, weight: 0.85, meaning: 'present perfect continuous — sustained activity' }],

  // ── Present progressive (in flight) ───────────────────────────────────────
  [/\b(is|are|am) (currently )?(raising|closing|hiring|expanding|acquiring|integrating|launching)\b/i,
    { aspect: VERB_TENSE_ASPECT.PRESENT_PROGRESSIVE, weight: 0.90, meaning: 'be + -ing — action underway' }],
  [/\b(is|are) (in|under) (negotiations|talks|due diligence|legal review)\b/i,
    { aspect: VERB_TENSE_ASPECT.PRESENT_PROGRESSIVE, weight: 0.88, meaning: 'process in progress' }],

  // ── Present analytic (stance, thesis, appraisal — not necessarily -ing) ───
  [/\b(is|are) betting (that )?\b/i,
    { aspect: VERB_TENSE_ASPECT.PRESENT_ANALYTIC, weight: 0.92, meaning: 'present stance — investor is betting / thesis' }],
  [/\b(bets on|betting on|wagers on|doubling down on|double(s|d)? down on)\b/i,
    { aspect: VERB_TENSE_ASPECT.PRESENT_ANALYTIC, weight: 0.88, meaning: 'present commitment language to a name or theme' }],
  [/\b(sees?|views?|viewing|positions?|positioning) .{0,50} as (a |the )?(same|similar|analog|parallel)\b/i,
    { aspect: VERB_TENSE_ASPECT.PRESENT_ANALYTIC, weight: 0.80, meaning: 'analogical framing — present analytic' }],
  [/\b(believes?|thinks?|argues?|expects?) (that )?\b/i,
    { aspect: VERB_TENSE_ASPECT.PRESENT_ANALYTIC, weight: 0.72, meaning: 'mental / thesis verb — present' }],
  [/\b(offers more|has more .{0,30} than|not as good as|better than|worse than)\b/i,
    { aspect: VERB_TENSE_ASPECT.PRESENT_ANALYTIC, weight: 0.78, meaning: 'comparative judgment — product or pipeline positioning' }],

  // ── Epistemic + parallel past (multi-actor narratives, e.g. OEM + supplier + competitor) ─
  [/\bhad no idea that\b/i,
    { aspect: VERB_TENSE_ASPECT.EPISTEMIC_PAST, weight: 0.92, meaning: 'past unawareness — sets up later revelation / pivot' }],
  [/\bdidn'?t know that\b/i,
    { aspect: VERB_TENSE_ASPECT.EPISTEMIC_PAST, weight: 0.88, meaning: 'past information gap' }],
  [/\b(was|were) doing (the same( thing)?|something similar|similar work)\b/i,
    { aspect: VERB_TENSE_ASPECT.PAST_CONTINUOUS, weight: 0.88, meaning: 'parallel ongoing action in the past (another actor)' }],
  [/\bbegan working with\b/i,
    { aspect: VERB_TENSE_ASPECT.COMPLETED_PAST, weight: 0.88, meaning: 'past inception of vendor / partner / supplier relationship' }],
  [/\bstarted working with\b/i,
    { aspect: VERB_TENSE_ASPECT.COMPLETED_PAST, weight: 0.86, meaning: 'past start of engagement' }],
  [/\bdropped (their |its |the )?(investment|stake|position|commitment)\b/i,
    { aspect: VERB_TENSE_ASPECT.COMPLETED_PAST, weight: 0.93, meaning: 'past divestment or withdrawal of capital / strategic commitment' }],
  [/\bdecided to build\b/i,
    { aspect: VERB_TENSE_ASPECT.COMPLETED_PAST, weight: 0.90, meaning: 'past decision to build in-house vs buy / partner' }],

  // ── Simple past — completed financing or corporate acts (headline grammar) ─
  [/\b(funded|co-?funded|led the round|co-?led|participated in|backed|invested in|wrote (a |the )?check|wired|closed on|announced|acquired|merged with)\b/i,
    { aspect: VERB_TENSE_ASPECT.COMPLETED_PAST, weight: 0.93, meaning: 'simple past — completed deal or event' }],
  [/\b(raised|closed|secured|completed|finalized) (a |the |their )?(seed|series [a-e]|round|financing|acquisition)\b/i,
    { aspect: VERB_TENSE_ASPECT.COMPLETED_PAST, weight: 0.91, meaning: 'past-tense round / exit milestone' }],
  [/\bannounc(ed|es|ing)? (the )?close of\b/i,
    { aspect: VERB_TENSE_ASPECT.COMPLETED_PAST, weight: 0.94, meaning: 'financing close announced (PR / wire)' }],
  [/\b(was|were) (funded|backed|acquired|sold to)\b/i,
    { aspect: VERB_TENSE_ASPECT.COMPLETED_PAST, weight: 0.88, meaning: 'passive past — completed corporate event' }],
];
...
const CONTEXT_MAP = {
  ...
  'industrial automation':    ['context_sector_robotics'],
  ...
  'real estate tech':         ['context_sector_proptech'],
  'property technology':      ['context_sector_proptech'],
...
const SIGNAL_CLASS_PRIORITY = [
  ...
  'diligence_signal',         // formal DD / data room — between interest and generic hiring
  'investor_rejection_signal',
  'regulatory_signal',
  'market_position_signal',
  'product_signal',
  'gtm_hiring_signal',        // typed hiring outranks generic hiring
...
const WHO_CARES_MAP = {
  ...
  failure_exit_signal:       { investors: true,  vendors: false, acquirers: true,  recruiters: false, partners: false },
  failure_learning_signal:   { investors: true,  vendors: false, acquirers: false, recruiters: false, partners: false },
  founder_psychology_signal: { investors: true,  vendors: false, acquirers: false, recruiters: false, partners: false },
  founder_excellence_signal: { investors: true,  vendors: false, acquirers: false, recruiters: true,  partners: false },
  founder_cunning_signal:    { investors: true,  vendors: true,  acquirers: false, recruiters: false, partners: true  },
  customer_delight_signal:   { investors: true,  vendors: true,  acquirers: false, recruiters: false, partners: true  },
  talent_magnet_signal:      { investors: true,  vendors: false, acquirers: false, recruiters: true,  partners: false },
  ...
  diligence_signal:          { investors: true,  vendors: false, acquirers: false, recruiters: false, partners: false },
  ...
  gtm_hiring_signal:         { investors: true,  vendors: true,  acquirers: false, recruiters: true,  partners: true  },
  engineering_hiring_signal: { investors: true,  vendors: true,  acquirers: false, recruiters: true,  partners: false },
...
const SIGNAL_TYPE_MAP = {
  ...
  gtm_hiring_signal:         'talent',
  engineering_hiring_signal: 'talent',
...
const INFERENCE_MAP = {
  ...
  diligence_signal: {
    likely_stage:        'series_a–growth',
    likely_need:         ['data_room', 'legal', 'financial_dd', 'cap_table', 'vendor_diligence'],
    likely_budget:       'low',
    urgency:             'high',
    strategic_direction: 'financing_close',
  },
  investor_rejection_signal: {
    likely_stage:        'any',
    likely_need:         [],
  ...
  gtm_hiring_signal: {
    likely_stage:        'seed–series_b',
    likely_need:         ['ATS', 'sales_tools', 'CRM', 'marketing_automation', 'sales_training', 'comp_plans'],
    likely_budget:       'medium',
    urgency:             'high',
  ...
  engineering_hiring_signal: {
    likely_stage:        'seed–series_b',
    likely_need:         ['ATS', 'developer_tools', 'cloud_infrastructure', 'security', 'on_call', 'equity_management'],
    likely_budget:       'medium',
    urgency:             'high',
    strategic_direction: 'product_scale',
  },
  enterprise_signal: {
...
const CONFLICT_PAIRS = [
  ...
  ['distress_signal',   'gtm_hiring_signal'],
  ['distress_signal',   'engineering_hiring_signal'],
  ['distress_signal',   'expansion_signal'],
  ['distress_signal',   'growth_signal'],
  ['distress_signal',   'gtm_signal'],
  ['efficiency_signal', 'hiring_signal'],
  ['efficiency_signal', 'gtm_hiring_signal'],
  ['efficiency_signal', 'engineering_hiring_signal'],
  ['efficiency_signal', 'expansion_signal'],
  ['fundraising_signal','distress_signal'],
  ['exit_signal',       'hiring_signal'],
  ['exit_signal',       'gtm_hiring_signal'],
  ['exit_signal',       'engineering_hiring_signal'],
...
const MULTI_SIGNAL_TRIGGERS = [
  ...
  /\bsimultaneously\b/i,
  /\bconcurrently\b/i,
  // Causal bridge: past clause + present/future thesis ("funded X last year, so it is betting…")
  /\b,\s*so\s+(it|they|he|she)\b/i,
  /\b,\s*so\s+it'?s\b/i,
  /\b,\s*so\s+they'?re\b/i,
  /\b(last year|last quarter|last month|in \d{4}),?\s+so\b/i,
  // Causal pivot without comma (informal / long sentences): "…so they dropped…"
  /\bso they (dropped|decided|stopped|learned|realized|found|began|started)\b/i,
