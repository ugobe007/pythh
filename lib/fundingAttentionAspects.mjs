...
export const FUNDING_ATTENTION_VERSION = 'funding-attention-v2';
...
export const FUNDING_ATTENTION_ASPECTS = Object.freeze({
  ...
  revenue_growth: {
    ...
    primary_signal: 'revenue_signal',
    label: 'Revenue or ARR growth cited as the raise trigger',
  },
  product_market_fit: {
    id: 'product_market_fit',
    theme: 'product-market fit',
    primary_signal: 'product_signal',
    label: 'Unique product-market fit cited as the raise trigger',
  },
  customer_access_partnership: {
    id: 'customer_access_partnership',
    theme: 'customer-access partnership',
    primary_signal: 'partnership_signal',
    label: 'Partnership that unlocks customer or channel access',
  },
});
...
const ASPECT_PATTERNS = Object.freeze({
  ...
  use_of_proceeds: [
    /\braises?\b.{0,120}\bto (scale|expand|build|launch|bring|enhance|tackle|transform|give|fund|accelerate|revolutionize|power|deploy|acquire|develop)\b/i,
  ...
  revenue_growth: [
    /\brevenue growth\b/i,
    /\b(arr|mrr)\b.{0,24}(grew|growth|doubled|tripled|up |increased)\b/i,
    /\b(grew|growing|doubled|tripled)\b.{0,20}\b(arr|mrr|revenue)\b/i,
    /\brevenue (grew|doubled|tripled|up )\b/i,
    /\b\d+(?:\.\d+)?x (revenue|arr)( growth)?\b/i,
    /\b(year[- ]over[- ]year|yoy) (revenue|arr) growth\b/i,
  ],
  product_market_fit: [
    /\bproduct[- ]market fit\b/i,
    /\b\bpmf\b/i,
    /\b(strong|clear|early|unique) (product[- ]market fit|pmf)\b/i,
    /\b(organic|inbound) (demand|waitlist|pull)\b/i,
    /\bcustomers? (pulling|demand|waitlist)\b/i,
    /\bnet revenue retention\b|\bnrr\b/i,
    /\bwillingness to pay\b/i,
    /\bunique (product|wedge|category) (fit|position)\b/i,
  ],
  customer_access_partnership: [
    /\b(distribution|channel|reseller|go-?to-?market) partners?(hip)?\b/i,
    /\bpartners?(hip)?s?\b.{0,48}\b(unlock|access to customers|customer access|distribution|channel|gtm)\b/i,
    /\b(unlocks?|opens?)\b.{0,40}\b(customer|distribution|channel|market) access\b/i,
    /\baccess to .{0,32}(customers|users|enterprises|hospitals|clinics|developers)\b/i,
    /\b(design|launch|channel) partners?\b/i,
    /\bpartners? with .{0,40}\bto (reach|sell to|distribute to|access) .{0,24}(customers|buyers|enterprises)\b/i,
  ],
});
...
export function extractFundingAttentionAspects(input) {
  ...
  for (const [id, spec] of Object.entries(FUNDING_ATTENTION_ASPECTS)) {
    if (id === 'partners' || id === 'customer_access_partnership') {
      if (FIRM_PARTNER_NOISE.test(text) && !/\b(strategic partnership|partnered with|partners with)\b/i.test(text)) {
        continue;
      }
    } else if (isNegated(id, text)) {
      continue;
    }
...
/** Preferred order for “why did this funding happen?” */
export const FUNDING_TRIGGER_PRIORITY = Object.freeze([
  'revenue_growth',
  'product_market_fit',
  'customer_access_partnership',
  'customer_growth',
...
/**
 * Rank announcement aspects into an explicit funding-trigger answer.
 * Primary reason prefers the three operator-requested triggers.
 */
export function inferFundingTriggers(extracted = {}) {
  const present = new Map((extracted.aspects || []).map((row) => [row.id, row]));
  const reasons = FUNDING_TRIGGER_PRIORITY
    ...
    .map((id) => {
      ...
      return {
        id,
        theme: row.theme,
        label: FUNDING_ATTENTION_ASPECTS[id]?.label || row.label,
        confidence: row.confidence ?? null,
      };
    });
  let primary = reasons[0] || null;
  if (!primary && extracted.cited) {
    primary = {
      id: 'cited_unclassified',
      theme: 'cited reason',
      label: 'Announcement cites a reason that did not match a trigger class',
      confidence: 0.4,
    };
  }
  return {
    primary: primary ? primary.id : 'unspecified',
    reasons,
    cited: Boolean(extracted.cited),
  };
}
